...
- [x] **M7.5-P1-E:** DEFERRED to M8 (Branch B chosen per M8/M9 Astro unified decision — Jinja2 webui replaced by Astro this week). Absorbed into M8 plan §9 acceptance criteria + pre-launch §10 M8 dependency annotation with explicit exit criteria (W3+W4 merged + `odoo-semantic-astro.service` active). **Runbook §6 Branch B.** (absorbed into M8 Astro — odoo-semantic-astro.service active)
...
**Status:** `[~]` M10A + M10.5 P1+P2 shipped; M10C substantially complete (Prometheus histogram shipped 2026-05-26 WI-D1; NAMEGET/reembed/audit shipped PR #159; nonce-CSP BLOCKED awaits Astro v5.1+); M10B P0 shipped (PR #200: plans schema + quota gating + usage dashboard); **M10B P1 engineering-complete on `feat/m10b-p1-billing`** (Entitlement Activation API + Polar webhook + claim-on-login + W1-W6 completion waves; single migration `m13_014` covers all billing schema — cancel_at_period_end + prices JSONB + terms_accepted_at + waitlist CHECK drop gộp vào m13_014; vendor-generic pipeline, self-serve cancel, admin config, legal pages + consent gate, billing dashboard); **legal CEO sign-off done (PR #224, DRAFT removed 2026-06-01); remaining: KYB + paid_checkout_enabled flip + Polar confirmation**; M10B P2-P3 pending. Prod reindex v8→v19 (comodel_name + mth.depends + migration m9_010) remains an OPS follow-up — admin run pending.
...
- [x] **Entitlement Activation API** — `EntitlementGrant` frozen dataclass + `grant/revoke/update` contract (ADR-0039 D3). `subscriptions` table (integer FK `plan_id`, `claimed_user_id`, `api_key_id`, `tenant_id`, `buyer_email`; NO per-row limits) + `billing_webhook_events` idempotency ledger; admin routes `POST/PATCH/GET /api/admin/entitlements` (mutating routes require `require_admin_with_fresh_mfa`; GET uses `require_admin`; all mutating routes carry @audit_action). (feat/m10b-p1-billing — m13_014; tool count stays 24)
- [x] **Polar.sh MoR adapter** — `POST /api/webhooks/polar` (public, HMAC-verified Standard Webhooks, fail-closed, idempotent ledger); `src/billing/polar.py` (verify_signature + EVENT_STATUS_MAP); product→plan map via `billing.polar_product_map` app_setting. (feat/m10b-p1-billing — m13_014; tool count stays 24)
- [x] **Self-serve issuance gated by payment** — claim-on-login: purchase keys a subscription to buyer email; `claim_subscription_for_user(user_id, email)` fires on verified-email login (signup email-verify / OAuth / password with email_verified=TRUE), upgrades free key in-place. (feat/m10b-p1-billing — m13_014; tool count stays 24)

**P1 completion waves (W1-W6, feat/m10b-p1-billing)**
- [x] **W1 — Schema hardening** — gộp vào `m13_014` (sections 6-8): `subscriptions.cancel_at_period_end` + `plans.prices` JSONB + guarded seed USD-only (Pro `{"USD":1900}`, Team `{"USD":3900}`; multi-currency display deferred to P2) (formerly m13_015); `webui_users.terms_accepted_at TIMESTAMPTZ` (formerly m13_016); drop hard-coded `waitlist_emails.plan` CHECK, replaced by DB-derived `_public_plan_slugs` (formerly m13_017). (tool count stays 24)
- [x] **W2 — Vendor-generic webhook pipeline** — `src/billing/webhook_pipeline.py` (`WebhookAdapter` + `run_webhook_pipeline`): 13-step pipeline vendor-agnostic, Polar is first adapter, second vendor is ~25 lines of glue. `src/billing/_db.py` (`slug_to_plan_id`): shared parameterised resolver. `src/billing/__init__.py` re-exports cleaned to vendor-neutral surface. `subscriptions.product_id` deferred to P2 (D2 extract-gradually). (tool count stays 24)
- [x] **W3 — Self-service cancel + backend routes** — `src/billing/polar_api.py` (outbound Polar REST client, `POLAR_API_KEY`, `billing.polar_api_base`, fail-closed: absent key → 503, error → 502); `activation.revoke_entitlement(voluntary=True)` schedules cancel-at-period-end without downgrade; `GET /api/account/subscription`; `POST /api/account/subscription/cancel` (calls Polar first, local flag set only on success, @audit_action). 8 new `billing.*` settings (free_plan_slug / unlimited_sentinel_slug / team_plan_slug / team_min_seats / polar_portal_url / polar_api_base / paid_checkout_enabled / polar_checkout_url_map). FLAG: Polar cancel endpoint constants in `src/billing/polar_api.py` must be confirmed against live Polar docs. (tool count stays 24)
- [x] **W4 — Admin plan price editing + waitlist** — `PATCH /api/admin/plans/{slug}` (`PlanPatch`) extended: `price_cents`, `currency`, `billing_interval`, `trial_days`, `prices`, `is_archived`. `GET /api/plans` exposes `prices` field. Waitlist allow-list DB-derived (`_public_plan_slugs`). (tool count stays 24)
...
- [x] **W6 — Legal pages + consent gate** — `/terms`, `/refund`, `/privacy` Astro pages; DRAFT badge removed per CEO sign-off 2026-06-01 (PR #224); external counsel pass recommended post-launch; `paid_checkout_enabled` stays `False` until KYB + Polar confirmation. Footer links. Required signup consent checkbox (`data-testid="signup-consent-checkbox"`) — submit disabled until checked; backend records `terms_accepted_at = NOW()` for password signup (`routes/signup.py`) and OAuth account-creation (`routes/oauth.py`). (tool count stays 24)
...
**PR #224 — Launch prep: install MCP-first, SEO/brand, English-only legal + CRD checkout consent (feat/launch-prep)**

Migration: **m13_017** (`subscriptions.buyer_type` + `withdrawal_waiver_accepted_at`). Tool count stays **24**.

- [x] **Install page MCP-first** — `/install/` static HTML + `InstallSnippets.astro` + homepage lead with `odoo-semantic-mcp` as primary 3-step path (marketplace → install → `/odoo-semantic-mcp:connect`); `odoo-semantic-skills` promoted as optional free (MIT) add-on; `plugins-data.ts` SSOT gains `installMcp` alias; `OpenSourcePlugins.astro` positions MCP as core connector.
- [x] **Brand SSOT** — `BRAND_FULL`/`BRAND_SHORT`/`BRAND_DEF` in `site/src/lib/constants.ts`; "Odoo Semantic MCP" full name primary across titles/H1/legal/first-mention/footer; "OSM" shorthand; unified logo `alt`; repaired Admin/Account/Tenant sidebar lockups; footer defines "OSM (Odoo Semantic MCP)" once.
- [x] **SEO + AI-discovery** — `@astrojs/sitemap` data-driven sitemap (replaces drift-prone static `sitemap.xml`; includes `/tools`/`/bootstrap`/`/terms`/`/privacy`/`/refund`; excludes auth/admin/account/tenant); JSON-LD (`Organization` in BaseLayout, `SoftwareApplication` on homepage, `Product`/`Offer` on pricing, `FAQPage`); canonical tags + OG/Twitter on auth pages; `public/llms.txt`; `robots.txt` disallows `/admin/`/`/account/`/`/tenant/`; `/benchmarks` gains SiteHeader/SiteFooter; cross-links "See all tools →"/"Full methodology →".
- [x] **English-only legal** — Vietnamese-language versions removed from legal pages (English-only in the public build; legal contact SSOT in `contact.ts`).
- [x] **Legal pages rewrite + DRAFT removal + ObfuscatedEmail + real entity** — real Viindoo Technology JSC details (reg-no 0201994665, registered address, hotline), effective date 2026-06-01, `support@`/`sales@`/`privacy@`/`legal@viindoo.com` in `contact.ts` SSOT; `ObfuscatedEmail.astro` (JS-assembled anti-harvest); DRAFT badges removed from `/terms`/`/privacy`/`/refund` on CEO sign-off 2026-06-01.
- [x] **Billing CRD checkout consent** — `m13_017_withdrawal_consent.sql` (`subscriptions.buyer_type` + `withdrawal_waiver_accepted_at`); buyer-type capture (business/consumer) pre-Polar-redirect; non-pre-ticked withdrawal-waiver checkbox (CRD Art. 22); consumer-without-waiver blocked; durable-medium email (CRD Art. 7(3)/8(8)); `POST /api/account/checkout-consent` + `GET /api/account/checkout-config` endpoints; `_billing-island.tsx` consent modal; `list_all` admin endpoint exposes consent cols; 10 new postgres integration tests.
- [x] **Tool count stays 24** — all changes are web/Astro/billing layer only; no new MCP tools.
...
- [x] **A3 — pgvector embeddings provenance:** `embeddings` rows gain `line_start`/`repo`/`repo_id` columns (migration m13_003); method/field chunks now store real source `.py` file path (was module dir). (commit 09c0560)
...
> **STILL DEFERRED:** WI-7 FERNET secrets manager + Postgres RLS (needs FORCE + non-owner read role), M10B P0 quota gating SHIPPED PR #200 — remaining P1-P3: Polar.sh MoR adapter + Entitlement Activation API + self-serve issuance gated by payment + multi-IdP + /pricing placeholder links (re-scoped from Stripe 2026-05-28, ADR-0039), nonce-CSP (BLOCKED — Astro v5.1+), recall benchmark (BLOCKED — Ollama SSL), §6 prod smoke, VN persona docs, OBS-2/OBS-3. M10C Prometheus histogram SHIPPED 2026-05-26.
...
- [x] **WI-4 — Mandatory fail-closed filter across user-data tools** — HIGH (architecture) **[RELEASE GATE — PASSED]** *(SHIPPED v0.10.0, PR #163)*
  - All **61 Cypher sites** (57 `src/mcp/server.py` + 4 `src/mcp/orm.py`) use the uniform fragment `($own IS NULL OR all(__p IN <alias>.profile WHERE __p IN $own OR __p IN $shared))` — a node is granted iff EVERY profile on it is own-or-shared; the `$profile_name IS NULL OR …` bypass is **removed**. `_latest_version()` + `find_override_point` scoped; the 3 pgvector queries filtered (`find_examples`/`find_style_override` by `profile_name = ANY(own ∪ shared)`; `suggest_pattern` exempt — global catalogue). The own/shared `all()` model (NOT `any(allowed)`) closes a shared-base leak the gate test caught. `profile_name` is now a **non-escalating narrowing filter** (only shrinks within own∪shared, never widens; an out-of-scope name fail-closes) — M13 supersedes ADR-0029. Postgres RLS deferred to WI-7 (needs `FORCE` + non-owner read role).
...
- [~] **WI-7 — `FERNET_KEY` → secrets manager + RLS enforcement cutover** — MED (security)
  ...
  - **[x] FERNET credstore cut (WI-7 OPS) — DONE (PR feat/wi7-fernet-credstore-cut):**
    - Active `LoadCredential=FERNET_KEY:/etc/credstore/FERNET_KEY` in both
      `odoo-semantic-webui.service` and `odoo-semantic-backup.service` (no longer commented out).
    - `docs/deploy/osm-fernet-run` wrapper (`systemd-run -p LoadCredential=`) for CLI
      (indexer/rotate-fernet/restore) — closes the CLI delivery gap.
    - FERNET_KEY removed from `.env`/`webui.env`. src.crypto `$FERNET_KEY` env fallback
      preserved for dev/non-systemd.
    - PREREQUISITE for prod: provision `/etc/credstore/FERNET_KEY` (root:root 0600) BEFORE
      enabling units (missing source = 243/CREDENTIALS hard-fail). Ops scripts handle this.
  - **OPS pending — RLS enforcement (SEPARATE effort — NOT in this PR):**
    - `[ ]` Tạo non-owner read role `osm_reader` + GRANT SELECT ON embeddings. (NOTE: grant
      footprint must be mapped before running — see runbook §5.14 preconditions.)
    - `[ ]` `ALTER TABLE embeddings FORCE ROW LEVEL SECURITY`. (NOTE: needs prior code-PR to
      wire indexer+webui `DELETE GUC='*'` and confirm no bypass path remains.)
    - `[ ]` Trỏ `PG_DSN` của process MCP service (:8002) sang `osm_reader` (env riêng cho :8002;
      indexer/admin/migrate giữ DSN owner). Lưu ý: code CHƯA đọc biến `PG_READ_DSN` riêng —
      override `PG_DSN` cho process MCP, KHÔNG đặt `PG_READ_DSN` rồi kỳ vọng có hiệu lực.
    - `[ ]` Verify smoke + `pytest tests/test_embeddings_rls.py`.
  - Acceptance (full): prod không đọc `FERNET_KEY` từ plain env file (✓ FERNET done); `osm_reader`
    bị filter bởi RLS (pending); audit row emitted trên unscoped admin query (pending RLS).
  - Dependency: P1–P4 functional; full reindex v8→v19 (§5.11g phải pass trước) — DONE.
