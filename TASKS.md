...
**Status:** `[~]` M10A + M10.5 P1+P2 shipped; M10C substantially complete (Prometheus histogram shipped 2026-05-26 WI-D1; NAMEGET/reembed/audit shipped PR #159; nonce-CSP BLOCKED awaits Astro v5.1+); M10B P0 shipped (PR #200: plans schema + quota gating + usage dashboard); M10B P1-P3 pending. Prod reindex v8→v19 (comodel_name + mth.depends + migration m9_010) remains an OPS follow-up — admin run pending.
...
- [ ] **Fix `/pricing` placeholder links** — `site/src/pages/pricing.astro` still has 2 dead placeholders (`formspree.io/f/placeholder-odoo-semantic`, `forms.gle/odoo-semantic-waitlist`). Replace with a real waitlist/checkout target. *(DEFERRED — tracked as follow-up)*
