...
- [x] **v8 era1 CLI parser runtime extraction** — *(2026-05-22, PR #160 WI-2 — bundled in "reindex-prep DB-impact wave v8→v19")* `parser_cli.py` now reads `openerp/` paths for v8/v9 and loads the static `commands` array from `spec_data/cli_commands_8.0.json` / `9.0.json` to produce `CLICommand` nodes. Also wired via `_PKG_PREFIX_REGISTRY` (ADR-0032 WI-6 follow-up). Test: `test_parser_cli.py` covers v8/v9 CLICommand extraction.
