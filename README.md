...
**Latest release:** v0.10.0 (2026-05-23) — Final-stretch wave (one PR): pre-reindex enrichment + agent-convenient output + multi-tenant enforcement gate. **Group A (reindex-forcing):** v19 split-ORM core coverage (Command/Domain/Constraint/Index from `odoo/orm/`); Neo4j enrichment (Method.docstring, Module manifest deps + repo_url/repo_id, USES_FIELD + DEPENDS_ON_FIELD edges, Field.string/help); pgvector provenance (real file_path + line_start + repo, migration m13_003). **Group B:** tool output now carries module/repo/file/intent so AI agents need not re-read source (resolve_field comodel/label/help, describe_module repo+path, etc.) + `module_inspect(method='dependencies')` transitive deps + repos + load order. **Group C (RELEASE GATE):** mandatory fail-closed tenant filter at 61+4 Cypher sites + pgvector + list tools (own/shared model — a tenant never sees another tenant's private node), proven by `tests/test_cross_tenant_isolation.py`; `resolve_tenant_scope`; ADR-0034 amendment; `profile_name` now advisory (tenant is the boundary). Tool count stays **24**. **OPS follow-up:** admin runs `python -m src.db.migrate` (m13_003) + full reindex v8→v19 (Group A data populates on re-index). v0.9.1 shipped the M13 pre-reindex DB/multi-tenant/git-integrity foundation. See CHANGELOG.md.
