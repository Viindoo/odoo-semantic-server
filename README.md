...
**Milestone 6 — "Scale Wow":** `[~]` Wave 1 + Wave 2 shipped 2026-05-10 — Wave 1: env harness (single source of truth `.env.example` + runtime version checks Neo4j ≥ 5.x / PG ≥ 16 / pgvector ≥ 0.8 + Python 3.12 enforcement + PEP 695 `PgConn` type alias) + P3 polish (per-profile advisory locks, PG `SimpleConnectionPool`, `--max-workers` parallel scan). **Wave 2: indexer thesis** — incremental indexer (`repos.head_sha` + `Module.last_commit_sha` tracking, git diff to skip unchanged repos / filter changed modules, `--full` flag for cleanup, force-push fallback) + auto-reseed pattern catalogue (`_SeedMeta` sha256 sentinel + `--force` bypass, wired into `index_profile()` end) + cross-profile parallel `--profile-workers` (ThreadPoolExecutor wraps `index_all` profile loop). **Wave 3 backlog (~10 WIs):** version_presets.py + pattern catalogue community path (PR template + 50→200 patterns) + `find_override_point` cross-version diff + EE_CONFUSION auto-detect via manifest license. **Wave 4 backlog (~5 WIs):** SSH auto-clone qua Ed25519 + known_hosts UI.  
**Milestone 7 — "Lifecycle Wow":** `[ ]` Backlog — track ecosystem evolution: `viindoo_equivalent_qname` auto-populate (graph traversal heuristic, M4.6→M6→M7), Module rename garbage collection (`--gc` flag, ADR-0007 D5 follow-up), cross-repo dependency change tracking, embedding cost observability metrics. Start sau khi M6 Wave 3 + Wave 4 đóng.  
