...
**Milestone 6 — "Scale Wow":** `[~]` Wave 1 shipped 2026-05-10 — env harness (Section H: single source of truth `.env.example` + runtime version checks Neo4j ≥ 5.x / PG ≥ 16 / pgvector ≥ 0.8 + Python 3.12 enforcement with ruff `UP` + PEP 695 `PgConn` type alias) + P3 polish (per-profile advisory locks, PG `SimpleConnectionPool`, `--max-workers` parallel scan). Wave 2+ ongoing: SSH auto-clone, incremental indexer, multi-version presets, pattern catalogue maintenance, EE auto-detect.  
