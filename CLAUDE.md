...
**2. Era-aware parser_python.py**: `parse_file()` luôn thử AST trước cho MỌI version; chỉ khi `ast.parse` raise `SyntaxError` mới fallback sang text-regex extract (`_parse_era1_text()`) + `FIELD_TYPES_LEGACY` (`function`, `related`, `dummy`, `sparse`) cho `_columns` dict (#285 — era1 v8/v9 vốn đã làm vậy, era2 v10+ nay khớp). Fallback recover cả `is_transient`/`is_abstract` (mirror era2). Log severity version-aware: DEBUG cho v8/v9 (expected Py2), WARNING cho v10+ (straggler). Chi tiết: [`docs/huong-dan-stack.md §Era parsing`](docs/huong-dan-stack.md#era-parsing).
...
- `0032` parser version-dispatch registry (M11 — `VersionRegistry(min_major, max_major, handler)` replaces hard-coded era branches in js/core/cli; supersedes prefix-selection part of ADR-0005; **#285 amendment: `_ERA_REGISTRY`/`_detect_era` REMOVED from parser_python.py — era selection there is now a parse-outcome decision (AST-first, text-regex fallback on `SyntaxError`), not a version lookup; `_PREFIX_REGISTRY`/`_OWL_ENABLED_REGISTRY`/`_PKG_PREFIX_REGISTRY` unchanged**)
