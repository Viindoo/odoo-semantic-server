...
def test_admin_data_wins_on_name_conflict(clean_pg):
    ...
    ``standard_viindoo_17`` is owned only by the Python seeder's Pass 1 —
    ...
    # standard_viindoo_17 is not pre-seeded by migration 0004 (SQL rescue path
    # only covers root odoo_N profiles).
    with clean_pg.cursor() as cur:
        cur.execute(
            "INSERT INTO profiles (name, odoo_version, description) VALUES (%s, %s, %s)",
            ("standard_viindoo_17", "17.0", "MANUAL — do not overwrite"),
        )
    # Run the Python seed — standard_viindoo_17 already exists → INSERT skipped →
