...
def _seed_access_key(pg_conn, name: str) -> int:
    """Insert a shared admin-managed access_key SSH pair (dummy material). Return id."""
    with pg_conn.cursor() as cur:
        cur.execute(
            "INSERT INTO ssh_key_pairs"
            " (name, public_key, private_key_encrypted, key_version, key_type)"
            " VALUES (%s, %s, %s, 1, 'access_key') RETURNING id",
            (name, f"ssh-ed25519 AAAA-{name}", f"enc-{name}"),
        )
        kid = cur.fetchone()[0]
    pg_conn.commit()
    return kid
...
# ---------------------------------------------------------------------------
# #7 (code review PR #183): SSH key is never a non-admin user choice
# ---------------------------------------------------------------------------


class TestSshKeyNotUserChosen:
    """A non-admin never selects the SSH key. Both add_repo (POST) and update_repo
    (PATCH) resolve the single shared admin-managed access key server-side and
    IGNORE any client-supplied ssh_key_id — so a tenant member cannot clone via a
    key they shouldn't control (e.g. another tenant's deploy_key). An admin retains
    the key-management surface and may target a specific stored key.
    """
    ...
    @pytest.mark.asyncio
    async def test_non_admin_ssh_repo_uses_shared_key_ignoring_client_id(self, migrated_pg):
        """Non-admin posts a decoy ssh_key_id; the new repo still uses the shared key."""
        from unittest.mock import patch

        _, user_t1_id, _, _ = _seed_users(migrated_pg)
        t1_id = _seed_tenant(migrated_pg, "w2_ssh_t1")
        _seed_profile(migrated_pg, "w2_ssh_profile_t1", tenant_id=t1_id)
        _seed_membership(migrated_pg, user_t1_id, t1_id)

        # Shared key = lowest id (list_ssh_keys orders by id); decoy is what the
        # caller tries to reference.
        shared_id = _seed_access_key(migrated_pg, "shared-key")
        decoy_id = _seed_access_key(migrated_pg, "decoy-key")
        assert decoy_id > shared_id

        app = create_app()
        cookies = await _login(app, "w2_user_t1", "UserPass123!")
        # Mock the async clone subprocess — we only assert the persisted repo row.
        with patch("src.web_ui.routes.repos.subprocess.Popen") as mock_popen:
            mock_popen.return_value.wait.return_value = 0
            async with _async_client(app, cookies=cookies) as client:
                resp = await client.post(
                    "/api/repos/repos",
                    json={
                        "profile": "w2_ssh_profile_t1",
                        "url": "git@github.com:example/w2-ssh-repo.git",
                        "branch": "17.0",
                        "ssh_key_id": str(decoy_id),  # caller tries another key
                    },
                )
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}: {resp.text}"
        ...
        with migrated_pg.cursor() as cur:
            cur.execute(
                "SELECT r.ssh_key_id FROM repos r JOIN profiles p ON r.profile_id = p.id"
                " WHERE p.name = 'w2_ssh_profile_t1' ORDER BY r.id DESC LIMIT 1"
            )
            row = cur.fetchone()
        assert row is not None, "New SSH repo must exist in DB"
        assert row[0] == shared_id, (
            f"non-admin SSH repo must use the shared key id={shared_id}, not the "
            f"client-supplied decoy id={decoy_id}; got {row[0]}"
        )

    @pytest.mark.asyncio
    async def test_non_admin_ssh_repo_clear_error_when_no_shared_key(self, migrated_pg):
        """With no shared access key configured, a non-admin SSH add gets a 400 hint."""
        _, user_t1_id, _, _ = _seed_users(migrated_pg)
        t1_id = _seed_tenant(migrated_pg, "w2_ssh_nokey_t1")
        _seed_profile(migrated_pg, "w2_ssh_nokey_profile", tenant_id=t1_id)
        _seed_membership(migrated_pg, user_t1_id, t1_id)
        ...
        assert "shared SSH key" in resp.text

    @pytest.mark.asyncio
    async def test_non_admin_patch_cannot_switch_to_decoy_ssh_key(self, migrated_pg):
        """PATCH parity with add_repo: a non-admin's client-supplied ssh_key_id is
        IGNORED, so they cannot repoint an SSH repo at another key (e.g. another
        tenant's deploy_key). The repo keeps its server-resolved shared key.
        Regression for the PATCH cross-tenant key-selection hole (code review #183).
        """
        _, user_t1_id, _, _ = _seed_users(migrated_pg)
        t1_id = _seed_tenant(migrated_pg, "w2_patch_ssh_t1")
        pid = _seed_profile(migrated_pg, "w2_patch_ssh_profile", tenant_id=t1_id)
        repo_id = _seed_repo(migrated_pg, pid, tenant_id=t1_id)
        _seed_membership(migrated_pg, user_t1_id, t1_id)

        shared_id = _seed_access_key(migrated_pg, "patch-shared-key")
        decoy_id = _seed_access_key(migrated_pg, "patch-decoy-key")
        # The repo is an SSH repo that already uses the shared key.
        with migrated_pg.cursor() as cur:
            cur.execute(
                "UPDATE repos SET url=%s, ssh_key_id=%s WHERE id=%s",
                ("git@github.com:example/w2-patch.git", shared_id, repo_id),
            )
        migrated_pg.commit()
        ...
        async with _async_client(app, cookies=cookies) as client:
            resp = await client.patch(
                f"/api/repos/repos/{repo_id}",
                json={"branch": "18.0", "ssh_key_id": decoy_id},  # caller tries another key
            )
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}: {resp.text}"

        with migrated_pg.cursor() as cur:
            cur.execute("SELECT ssh_key_id, branch FROM repos WHERE id=%s", (repo_id,))
            row = cur.fetchone()
        assert row is not None
        assert row[0] == shared_id, (
            f"non-admin PATCH must IGNORE client ssh_key_id: repo must keep the shared "
            f"key id={shared_id}, not switch to decoy id={decoy_id}; got {row[0]}"
        )
        assert row[1] == "18.0", "the non-key field (branch) must still be updated"

    @pytest.mark.asyncio
    async def test_admin_patch_can_set_specific_ssh_key(self, migrated_pg):
        """Admin retains the key-management surface: an admin PATCH may target a
        specific stored key (the non-admin restriction must not over-block admins).
        """
        _seed_users(migrated_pg)
        pid = _seed_profile(migrated_pg, "w2_patch_admin_profile", tenant_id=None)
        repo_id = _seed_repo(migrated_pg, pid, tenant_id=None)
        key_a = _seed_access_key(migrated_pg, "admin-key-a")
        key_b = _seed_access_key(migrated_pg, "admin-key-b")
        with migrated_pg.cursor() as cur:
            cur.execute(
                "UPDATE repos SET url=%s, ssh_key_id=%s WHERE id=%s",
                ("git@github.com:example/w2-admin-patch.git", key_a, repo_id),
            )
        migrated_pg.commit()
        ...
        async with _async_client(app, cookies=cookies) as client:
            resp = await client.patch(
                f"/api/repos/repos/{repo_id}",
                json={"ssh_key_id": key_b},
            )
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}: {resp.text}"
        with migrated_pg.cursor() as cur:
            cur.execute("SELECT ssh_key_id FROM repos WHERE id=%s", (repo_id,))
            row = cur.fetchone()
        assert row[0] == key_b, f"admin PATCH must set the chosen key id={key_b}, got {row[0]}"
