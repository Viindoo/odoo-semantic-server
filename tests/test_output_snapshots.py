...
# ---------------------------------------------------------------------------
# G4 contract — _resolve_method override chain cap with disclosure
# ---------------------------------------------------------------------------

def test_resolve_method_override_chain_cap_contract(snapshot_db):
    """G4 contract: override chain has cap at LIST_PREVIEW_MAX_ITEMS.

    With snapshot_db seeded data (1 entry), chain fits within cap — no
    disclosure line expected. The cap+disclosure path is exercised in
    test_mcp_impact_analysis.py which seeds 25+ view modules.
    """
    from src.mcp.server import _resolve_method
    ...
    assert any("Override chain" in ln for ln in lines), "Missing Override chain header"
    # With only 1 module in fixture, no truncation — no disclosure line expected.
    assert not any(
        "... and" in ln and "more (use find_override_point" in ln for ln in lines
    ), "Unexpected disclosure line when chain fits within cap"
...
def test_find_deprecated_usage_overflow_discloses_more_available(spec_snapshot_db):
    ...
    # Simulate overflow: pass records equal to cap + overflow=True
    fake_records = [
        {
            "module": f"mod_{i}",
            "model": "test.model",
            "method": "method_x",
            "deprecated_symbol": "old_api.foo",
            "status": "deprecated",
            "replacement": "new_api.bar",
            "repo": None,
        }
        for i in range(LIST_PREVIEW_MAX_ITEMS)
    ]
    result = _format_deprecated_usage(fake_records, "17.0", overflow=True)
    lines = result.splitlines()

    # Header must show N+ (overflow marker)
