...
Business invariant:
  ...
  redirects the user to /admin/login before the OAuth handler runs makes the
...
Implementation-agnostic:
  ...
  to an /admin/auth/* path is not bounced to /admin/login.
...
What it rejects:
  - Any 3xx whose Location contains /admin/login
...
class TestOAuthAnonymousAccess:
    """Anonymous GET to /admin/auth/* must not redirect to /admin/login."""
    ...
    def test_callback_route_not_intercepted_by_admin_middleware(
        ...
        does NOT bounce the user back to /admin/login before the handler runs.
        """
        code, location = _get_first_response(f"{astro_server}{path}")
        if 300 <= code < 400:
            assert "/admin/login" not in location, (
