...
to /login (canonical login URL — WI-3 rename; /admin/login 301-redirects here).
...
Regression covers:
  - /account              → 302 to /login (canonical)
  - /account/api-keys     → 302 to /login (canonical)
"""
import pytest
...
class TestAccountAnonRedirect:
    def test_anon_account_root_redirects_to_login(self, astro_server, page):
        """GET /account (no auth cookie) → ends on /login (canonical login page)."""
        page.goto(f"{astro_server}/account", wait_until="load")
        # Accept /login as canonical; /admin/login is still valid during
        # transition if a browser cached the old 302 and follows it.
        assert "/login" in page.url, (
            f"Expected /login in final URL, got {page.url!r}"
        )
    ...
    def test_anon_account_api_keys_redirects_to_login(self, astro_server, page):
        """GET /account/api-keys (no auth cookie) → ends on /login (canonical)."""
