  ...
  return (
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-4">
        <h2 class="text-base font-semibold text-gray-800 mb-3">Create New Tenant</h2>
        <form onSubmit={handleCreateTenant} class="flex gap-3 items-end">
          <div class="flex-1">
            <label class="block text-xs font-medium text-gray-600 mb-1">Tenant Name</label>
            <input
              type="text"
              value={newTenantName}
          ...
          <button
            ...
            class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-4 py-3 border-b border-gray-100">
          <h2 class="text-base font-semibold text-gray-800">All Tenants</h2>
        </div>
        {tenants.length === 0 ? (
          <div class="px-4 py-8 text-center text-sm text-gray-400">No tenants yet. Create one above.</div>
        ) : (
          <table class="w-full text-sm">
            <thead class="bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
              <tr>
                ...
                <th class="px-4 py-2 text-center">Repos</th>
                <th class="px-4 py-2 text-center">Profiles</th>
            ...
            <tbody>
              {tenants.map((t) => (
                <tr
                  key={t.id}
                  class={`border-t border-gray-100 cursor-pointer hover:bg-blue-50 ${selectedTenant?.id === t.id ? 'bg-blue-50' : ''}`}
                  onClick={() => setSelectedTenant(selectedTenant?.id === t.id ? null : t)}
                >
                  <td class="px-4 py-2 font-medium text-gray-900">
                    {editingId === t.id ? (
                      <span class="flex gap-2 items-center" onClick={(e) => e.stopPropagation()}>
                        <input
                          type="text"
                          value={editName}
                  ...
                  <td class="px-4 py-2 text-center">
                    <span class={`text-xs px-2 py-0.5 rounded-full font-medium ${t.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      {t.active ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td class="px-4 py-2 text-center text-gray-600">{t.member_count}</td>
                  <td class="px-4 py-2 text-center text-gray-600">{t.repo_count}</td>
                  <td class="px-4 py-2 text-center text-gray-600">{t.profile_count}</td>
                  <td class="px-4 py-2 text-right space-x-2" onClick={(e) => e.stopPropagation()}>
                    <button
      ...
      {/* Tenant detail panel */}
      {selectedTenant && (
        <div class="bg-white rounded-xl border border-blue-200 p-4 space-y-5">
          <h2 class="text-base font-semibold text-gray-800">
            Detail: <span class="text-blue-700">{selectedTenant.name}</span>
          </h2>
          ...
          <div>
            ...
            {loadingMembers ? (
              ...
              <p class="text-xs text-gray-400">No members. Add one below.</p>
            ) : (
              <table class="w-full text-sm mb-3">
                <thead class="text-xs text-gray-500 bg-gray-50">
                  <tr>
                    <th class="px-3 py-1 text-left">Username</th>
                    <th class="px-3 py-1 text-left">Email</th>
                    <th class="px-3 py-1 text-center">Role</th>
                    <th class="px-3 py-1 text-right">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {members.map((m) => (
                    <tr key={m.user_id} class="border-t border-gray-100">
                      <td class="px-3 py-1 font-medium">{m.username}</td>
                      <td class="px-3 py-1 text-gray-500">{m.email ?? '-'}</td>
                      <td class="px-3 py-1 text-center">
                        <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{m.role}</span>
                      </td>
            ...
            {availableUsers.length > 0 && (
              <form onSubmit={handleAddMember} class="flex gap-2 items-end mt-2">
                <div>
                ...
                <button
                  ...
                  class="px-3 py-1.5 bg-blue-600 text-white text-xs rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50"
                >Add Member</button>
              </form>
            )}
            {availableUsers.length === 0 && members.length > 0 && (
              <p class="text-xs text-gray-400 mt-2">All available users are already members.</p>
            )}
          </div>
