...
| `INDEXER_JOB_QUEUED_TTL_SECONDS` | 600s | - | `src/constants.py` | Age after which the Web UI start-up sweep (`JobStore.mark_dead_jobs`) marks a `queued` indexer job that never started (`started_at` NULL) as `error`, whatever its pid: a live child reports `running` within seconds, so a job still queued past this age never started or died before its first report, and its recorded pid may be gone, reused after a reboot or another user's (#381). Younger rows are left alone so a spawn in progress is never expired. An admin can also clear one at once with `POST /api/jobs/{id}/reset` (same rule, `job_registry.job_staleness`, which also clears a job whose pid was reused: the machine booted, or the pid's process started, more than 120 s after the job started; the slack absorbs wall-clock steps, and a forward clock step larger than that while a job runs can mark that live job error - see `_START_SLACK_S`). |
