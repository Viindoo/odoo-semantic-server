...
| `INDEXER_JOB_QUEUED_TTL_SECONDS` | 600s | - | `src/constants.py` | Age after which the Web UI start-up sweep (`JobStore.mark_dead_jobs`) marks a `queued` indexer job with no pid as `error`. The Web UI records the child's pid right after the spawn, so a job still queued without a pid past this age never started (or died before its first report) and nothing else would clear it (#381). Younger rows are left alone so a spawn in progress is never expired. An admin can also clear one at once with `POST /api/jobs/{id}/reset`. |
