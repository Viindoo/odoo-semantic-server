...
| `embedding_sweep:mass_retire` / `embedding_sweep:total_wipe` (`embedding sweep at <v>: N of M embedding group(s) ... for profile(s) <p>`; stderr thêm dòng `embedding_orphans_held: <v>: N group(s) of profile(s) <p>`; profile không còn repo nào ở version thì attention nằm trên mọi repo của version) | hơn 50% (và >= 20) nhóm embedding `(module, profile)` của version là orphan, hoặc ledger không còn module nào present ở version đó | đối chiếu `embedding_orphans_held` của `lifecycle-audit --all --json` với git (module còn được repo của profile đó track không?); nếu đúng là rác: một lần `index-repo --profile <p> --allow-mass-retire` |
