...
**Amendment 2 (issue #362, ADR-0054):** Three statements in this ADR described intent the original implementation never delivered, or have since been superseded by ADR-0054. This amendment records what changed; it does not rewrite the original text above.

- **`:148` ("Framework base seeding: odoo/tests walk per version creates TestHelper nodes"):** this described an intent that was never implemented. No walk of `odoo/tests` ever ran per version; `seed_framework_helpers(odoo_version)` used its version argument only to stamp the written node, then wrote all eleven entries of one flat, version-blind `_FRAMEWORK_BASES` dict identically under every indexed version - the same wrong menu at v8 and at v19. This is the root cause of issue #362. ADR-0054 implements the walk this line always claimed to exist, as a verification oracle (AST parse of the real `odoo/tests/common.py` / `form.py`, with a text-regex fallback) layered over a curated, per-era table - not a bare walk with no fallback, since a source root is not reachable on every seeding path (see ADR-0054 Decision §3).

- **`:222-231` (Tool 4 `test_base_classes`: "Authoritative framework base-class menu (... TransactionCase, SavepointCase, HttpCase, Form, SingleTransactionCase per version)"):** the "per version" claim is true only as of ADR-0054. Before it, the menu was version-invariant by construction (see above) and several of the individual per-class facts this line implies were also wrong (SavepointCase's real v8-v16 window and non-alias status, TreeCase's real v11-v14 window, HttpCaseCommon's real v14-only window, Form/O2MForm's real v12+ lower bound - see ADR-0054 Context for the full corrected set).

- **`:96` (CRITICAL-1, MERGE key rationale: "Framework helpers (TestHelper with module='@framework') have null file_path and no repo"):** this described the then-current seeding behaviour, not a schema constraint. As of ADR-0054, `file_path` and `line` are populated where a real AST parse enriched them (via the `index-core` path only) and remain `null` otherwise - both are non-key properties, written with coalesce-ON-MATCH so a source-less reseed never erases prior enrichment. The MERGE key itself, `(name, module, file_path, repo, odoo_version)` for TestClass and the framework-sentinel `(name, module, odoo_version)` for TestHelper, is unchanged; the CRITICAL-1 key statement stands as originally written.

- **`:296` (Consequences/Risk, "Framework base seeding: relies on parsing `odoo/tests/*.py`. If seeding incomplete, tools fall back to static known data. Low risk (framework bases change rarely)."):** the "change rarely" premise is refuted by the full v8-v19 survey. Of the eleven adjacent-major version boundaries between v8 and v19, the framework base-class menu (membership, deprecation status, or source location) actually changes at six: v9|v10 (source prefix `openerp/`->`odoo/`), v10|v11 (`TreeCase` added), v11|v12 (`Form`/`O2MForm` added), v13|v14 (`HttpCaseCommon`/`HttpSavepointCase` added), v14|v15 (`TreeCase` and `HttpCaseCommon` removed; `SavepointCase`/`HttpSavepointCase` become deprecated), and v16|v17 (`SavepointCase`/`HttpSavepointCase` removed; `Form`/`O2MForm` relocate to `odoo/tests/form.py`). A menu that changes at more than half the surveyed boundaries is not a low-churn artifact that tolerates a stale fallback - it is exactly the kind of data that needs the parse-verified curation ADR-0054 introduces.
