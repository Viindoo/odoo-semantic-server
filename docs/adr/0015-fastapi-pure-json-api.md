...
5. **`GET /api/auth/verify`** — new endpoint consumed by Astro middleware to check if the session cookie is valid before serving protected pages. Returns `{"ok": true, "username": "...", "is_admin": <bool>, "email": "...", "is_tenant_admin": <bool>}` (200) or `{"ok": false, "error": "not_authenticated"}` (401). `email` and `is_tenant_admin` were added in PR #216 so the Astro UI can render role-aware navigation (avatar dropdown Tenant Settings item) without an extra round-trip. This is the Astro-to-FastAPI auth proxy pattern.
