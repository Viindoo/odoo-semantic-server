...
**Status:** Accepted  
**Date:** 2026-05-18  
**Milestone:** M9 ops follow-up

---

## Context

The initial production deployment ran as a personal login user (UID 1000) with:
- App directory inside the personal home (`~/git/odoo-semantic-mcp/`)
- Venv inside the personal home (`~/.venv/odoo-semantic-mcp/`)
- Config mixed inside the app directory
- Indexed repo clones scattered under personal XDG data dir

This created several operability problems:

1. `ProtectHome=true` in systemd units made the service home invisible, causing startup failures on services that expected the venv at `~/.venv/...`.
2. Backup CLI staging (pg_dump output + tar.gz) exhausted `/tmp` on hosts where tmpfs is sized to RAM (8 GB tmpfs, 8 GB+ postgres.sql = OOM).
3. No separation between the operator's personal shell activity and the long-running service processes.
4. Backup/restore tooling could not assume stable absolute paths for config files and clone directories.

The migration moved all service-owned resources under a dedicated system user `odoo-semantic` and standardised the directory layout. This ADR documents the decisions made, the resulting layout, and the operational gotchas discovered during migration.
...
Production runs as a dedicated system user `odoo-semantic` created with `--system --shell /bin/bash --create-home`.  
The user must be in the `docker` group so the backup CLI can exec into containers.
...
Explicit `ReadWritePaths=` lines in each unit file cover the writable paths the service actually needs
(e.g. clone directory, backup output directory).
...
output (can exceed 8 GB for a mature production DB) and the resulting archive. On hosts where
`/tmp` is a tmpfs sized to physical RAM, both the intermediate file and the final archive compete
for that same memory-backed filesystem. The resulting RAM spike can cause the OS to kill the
backup process (OOM) or exhaust available tmpfs space.
...
The backup runbook (`docs/deploy/backup-runbook.md`) and the shipped unit file
(`docs/deploy/odoo-semantic-backup.service`) should carry this line.
...
A deployment can relocate the app directory (e.g. from a personal home to the service account home)
without running `docker compose down/up`, provided the new directory basename is identical and the
named Docker volumes remain intact.
...
After rsync'ing indexed repo clones to a new path, the `repos` table in PostgreSQL must be updated
to reflect the new `local_path` for every repo row. Failure to do this causes the indexer to
look for repos at the old path and fail silently (the profiled repo is skipped, no re-index error
is raised).
...
- **Predictable paths for backup/restore tooling:** absolute paths in `docs/deploy/odoo-semantic-mcp.service`
  and the backup unit are stable across operator changes (no dependency on `$HOME` of a specific
  person).
...
Deployments still running as a personal login user can continue to function. The migration is
recommended but not forced. To migrate an existing personal-user deployment to the system-user
layout:
