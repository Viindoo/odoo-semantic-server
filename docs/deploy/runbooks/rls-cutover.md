...
- Migration `m13_006_plans.sql` đã được apply — bao gồm `GRANT SELECT ON plans TO osm_reader` và `GRANT SELECT, INSERT, UPDATE ON usage_counter TO osm_reader`. Nếu chưa apply, chạy `python -m src.db.migrate` trước (xem `docs/deploy/runbooks/post-pr-ops.md §Action 0`).
