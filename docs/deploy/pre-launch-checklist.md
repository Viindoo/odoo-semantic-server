...
| 6 | `lint_check` | `lint_check("sale", "17.0")` | Lint rule hits list hoặc "no violations" | `[x]` 2026-05-14 — V0 fuzzy matcher works; P2 catalogue gap noted (M7.5-P2-LINT) ; 2026-06-11 — V0.5 pattern-first matcher + `code_pattern` data verified: SQL-injection (tuple-`%`) → `[pattern]` E8501/W8140. |
...
| 21 | `resolve_orm_chain` | `resolve_orm_chain("sale.order", "partner_id.country_id.code", "17.0")` | Hop-by-hop dotted path walk; `BROKEN` line at first unresolved hop with reason | `[x]` 2026-06-11 — Verified post ADR-0048 deploy: both repro paths resolve with no hang (`product.product`→`categ_id`; `sale.order`→`order_line.product_id.categ_id`); `[pattern]` label confirmed on the SQL-injection `lint_check` smoke. |
| 22 | `validate_domain` | `validate_domain("sale.order", "[('partner_id.country_id', '=', 'VN')]", "17.0")` | Per-term field-path + operator validation; version-aware operator set | `[x]` 2026-06-11 — Verified post ADR-0048 deploy: per-term field-path + version-aware operator validation correct on a multi-hop domain. |
