...
<!-- verified: systemctl status certbot.timer → active (waiting) với next trigger được lên lịch -->
...
<!-- admin verify: api_keys count khớp số key đã cấp; không có key lạ -->
...
<!-- verified: manual backup run via systemd backup unit succeeded; bundle chứa manifest.json + postgres.sql (sha256 verified). Result=success per systemd. Nightly timer scheduled (Persistent=true). -->
...
<!-- not verified: FERNET_KEY present in `<APP_DIR>/.env` but "secrets manager separate from server disk" requires admin confirmation of offsite backup -->
...
| 12 | `model_inspect` | `model_inspect(model="sale.order", method="fields", odoo_version="17.0")` | Superset router: delegates to underlying field enumeration; discriminator field in structuredContent | `[ ]` (M11 Wave D — ADR-0028; pending prod smoke) |
| 13 | `module_inspect` | `module_inspect(name="sale", method="summary", odoo_version="17.0")` | Superset router: delegates to `describe_module`; discriminator in structuredContent | `[ ]` (M11 Wave D — ADR-0028; pending prod smoke) |
| 14 | `entity_lookup` | `entity_lookup(kind="field", model="sale.order", field="amount_total", odoo_version="17.0")` | Auto-detects entity type (model/field/method/view/module) and routes to appropriate superset tool | `[ ]` (M11 Wave D — ADR-0028; pending prod smoke) |
...
| 21 | `resolve_orm_chain` | `resolve_orm_chain("sale.order", "partner_id.country_id.code", "17.0")` | Hop-by-hop dotted path walk; `BROKEN` line at first unresolved hop with reason | `[ ]` (M10.5 Phase 2 — v0.8.0; **re-verify after deploy** using repro case #273: `resolve_orm_chain("product.product","categ_id","17.0")` must return < 5s; also verify `resolve_orm_chain("sale.order","order_line.product_id.categ_id","17.0")` < 5s. `[pattern]` label must appear on SQL injection `lint_check` smoke. Do NOT tick until post-deploy smoke confirmed.) |
| 22 | `validate_domain` | `validate_domain("sale.order", "[('partner_id.country_id', '=', 'VN')]", "17.0")` | Per-term field-path + operator validation; version-aware operator set | `[ ]` (M10.5 Phase 2 — v0.8.0; re-verify after deploy — see row 21 note) |
| 23 | `validate_depends` | `validate_depends("sale.order", "_compute_amount_total", "17.0")` | Validates each `@api.depends` path; flags depends-on-`id`; era1 note for v8/v9 | `[ ]` (M10.5 Phase 2 — v0.8.0; re-verify after deploy — see row 21 note) |
| 24 | `validate_relation` | `validate_relation("sale.order", "partner_id", "res.partner", "17.0")` | Asserts field is relational with comodel matching `res.partner`; reports actual comodel on mismatch | `[ ]` (M10.5 Phase 2 — v0.8.0; **re-verify after deploy** with both cases: (a) MATCH `validate_relation("sale.order","partner_id","res.partner","17.0")` must return OK in < 5s; (b) MISMATCH (exhaustive-negative) `validate_relation("sale.order","partner_id","account.move","17.0")` must return actual-comodel mismatch message in < 5s - this was TIMEOUT before the per-hop pruning fix (PR #275 r3 CRITICAL-1). Verify BEFORE cleanup script runs (Wave-0 interim window). Do NOT tick until both cases pass on prod. See ADR-0048 D8 amendment.) |
...
<!-- admin verify: site/dist/server/entry.mjs tồn tại trong APP_DIR -->
...
<!-- verified: SIGKILL webui → service auto-restart trong ~5s (Main PID đổi sau restart). Journal shows "Killed unit cgroup ... with SIGKILL" → "Started server process" sequence. Auto-restart policy works. -->
...
> **DRIFT ALERT — FORCE ROW LEVEL SECURITY:** Migrating `m13_004_embeddings_rls.sql` only ENABLES row-level security policies (armed-dormant state). To activate RLS enforcement on the `embeddings` table, the operator must separately run `sudo ops/rls_cutover.sh` (manual OPS step). Migration alone does NOT automatically flip `ALTER TABLE ... FORCE ROW LEVEL SECURITY`. Until this step is done, the MCP server `:8002` continues reading with the current DSN (no RLS filtering). For multi-tenant deployments, RLS cutover must complete before customer data isolation is active. See `docs/deploy/runbooks/rls-cutover.md` for step-by-step procedure and verification checklist.
...
15. **§6 Tools 15-21 prod smoke (PENDING 2026-05-17)** — The `describe_module` tool (M9 W-OSM Wave 1) needs an end-to-end smoke against the production MCP endpoint via Claude Code or another MCP client. Example: `describe_module(name="sale", odoo_version="17.0")` should return manifest + model/view/JS patch counts. **Note:** 6 flat tools (`list_fields`, `list_methods`, `list_views`, `list_owl_components`, `list_qweb_templates`, `list_js_patches`) were removed in v0.6 per ADR-0028; verify entity enumeration via the superset tools (tools 12-14: `model_inspect`, `module_inspect`, `entity_lookup`) instead. All 7 tools are code-complete + unit-tested; no expected failures.
