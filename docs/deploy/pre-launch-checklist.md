...
- [ ] `curl -I https://<domain>/health` → HTTP 200, header `Strict-Transport-Security` có mặt **(P1 BLOCKED 2026-05-14 — Nginx config missing `add_header Strict-Transport-Security` directive; see `docs/m7.5-verification-issues.md` and M7.5-P1-D in TASKS.md M8)**
...
| 5 | `find_examples` | `find_examples("compute tax based on partner country")` | 5 results với file path + score *(skip nếu `--no-embed`)* | `[ ]` **P1 BLOCKED** — Ollama SSL CERTIFICATE_VERIFY_FAILED on production embedder; see M7.5-P1-A |
...
| 7 | `lookup_core_api` | `lookup_core_api("name_get", "17.0")` | Status (active/deprecated/removed) + description | `[ ]` **P1 BLOCKED** — CoreSymbol index gap for `name_get`; see M7.5-P1-B |
| 8 | `api_version_diff` | `api_version_diff("name_get", "16.0", "17.0")` | Diff giữa 2 version — thay đổi signature hoặc status | `[ ]` **P1 BLOCKED** — same CoreSymbol gap as #7 |
...
| 11 | `cli_help` | `cli_help("server", "--gevent-port", "17.0")` | Flag description + version added/removed | `[ ]` **P1 BLOCKED** — CLICommand index gap for `--gevent-port`; see M7.5-P1-C |
| 12 | `suggest_pattern` | `suggest_pattern("computed field cross-model partner_id")` | 3-5 PatternExample với code snippet + gotchas | `[ ]` **P1 BLOCKED** — same Ollama SSL as #5; see M7.5-P1-A |
...
**Sign-off summary 2026-05-14:** 9/14 PASS, 5/14 P1 BLOCKED (4 unique root causes — Ollama SSL × 2, CoreSymbol gap × 2, CLI gap × 1). Production canary `resolve_model("sale.order", "17.0")` PASS. Full report: `docs/m7.5-batch1-mcp-signoff.md`.
...
- [ ] Unauthenticated GET `/repos` → 302 redirect đến `/login`:
  `curl -I http://127.0.0.1:8003/repos` → `Location: /login` **(P1 BLOCKED 2026-05-14 — `https://odoo-semantic.viindoo.com/admin/repos` → HTTP/2 404, không redirect. Web UI service có thể không chạy hoặc nginx upstream chưa configure. Có thể là M8 dependency vì M8 Astro thay thế Jinja2 admin. See M7.5-P1-E.)**
- [ ] POST `/login` với sai mật khẩu → flash error (không grant session):
  ...
  → phải thấy error indicator trong redirect URL **(admin SSH verify — depends on M7.5-P1-E)**
- [ ] GET `/logout` clears session → tiếp theo request tới `/` → 302 `/login` **(admin SSH verify — depends on M7.5-P1-E)**
- [ ] `WEBUI_SESSION_SECRET` đã set trong `webui.env` (không dùng auto-generated ephemeral secret):
  `sudo grep WEBUI_SESSION_SECRET /etc/odoo-semantic/webui.env` → non-empty value **(admin SSH verify)**
