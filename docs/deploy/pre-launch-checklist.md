...
| 24 | `validate_relation` | `validate_relation("sale.order", "partner_id", "res.partner", "17.0")` | Asserts field is relational with comodel matching `res.partner`; reports actual comodel on mismatch | `[ ]` (M10.5 Phase 2 — v0.8.0; re-verify after deploy with MISMATCH case: `validate_relation("sale.order","partner_id","account.move","17.0")` must report actual comodel mismatch, not hang — see row 21 note) |
