...
9. **§10.5 CSP + Permissions-Policy headers missing** — `curl -sv https://odoo-semantic.viindoo.com/` shows no `Content-Security-Policy` or `Permissions-Policy` headers in nginx response. **Resolved post-launch (PR #118):** CSP + Permissions-Policy are now emitted by the application middleware (Astro `site/src/middleware.ts` for SSR pages; FastAPI `src/web_ui/app.py::_SecurityHeadersMiddleware` for `/api/*`). The nginx/Caddy edge intentionally does NOT add CSP — per W3C CSP3 §4.1 multiple CSP headers are intersected (most restrictive wins per directive), which would silently strip middleware-granted permissions (e.g. hCaptcha origins on /signup). Single source of truth = application middleware. Verify post-deploy: `curl -sI https://odoo-semantic.viindoo.com/signup | grep -i content-security`.
