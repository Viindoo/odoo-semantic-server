...
- [ ] Port 9999 TLS hoạt động (nếu dùng variant): `curl -I https://<domain>:9999/health` → 200
  - *Port 9999 dành cho install page public; xem nginx.conf.example §Port 9999 variant*
...
- [ ] `https://<domain>:9999/install/` (hoặc `:443/install/`) → load thành công, không 404
