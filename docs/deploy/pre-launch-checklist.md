...
| 5 | `find_examples` | `find_examples("compute tax based on partner country")` | 5 results với file path + score *(skip nếu `--no-embed`)* | `[x]` **P1-A RESOLVED 2026-05-14** — Embedder URL port `:9999` (closed) → drop port (port 443). MCP restarted. Awaiting client-side smoke. |
...
| 12 | `suggest_pattern` | `suggest_pattern("computed field cross-model partner_id")` | 3-5 PatternExample với code snippet + gotchas | `[x]` **P1-A RESOLVED 2026-05-14** — Bundled fix with #5. Awaiting client-side smoke. |
...
**Sign-off summary 2026-05-14 (hotfix applied):** 13/14 PASS (server-side verified) + 1 PARTIAL (#8 `api_version_diff` needs Tier 2 v16 index). Tools #5 + #12 require client-side MCP smoke test post-restart. P1-D HSTS verified. All P1 root causes resolved except P1-E (deferred to M8 per Branch B). Full report: [`docs/m7.5-verification-issues.md`](../m7.5-verification-issues.md) Resolution Stamps + [`docs/m7.5-batch1-mcp-signoff.md`](../m7.5-batch1-mcp-signoff.md).
