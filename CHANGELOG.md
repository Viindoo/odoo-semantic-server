...
- **RelaxNG XML validation → `:LintViolation` nodes** (WI-E) — `src/indexer/parser_xml.py` post-parse step validates each view against vendored Odoo LS RelaxNG schemas (NOTICE file included). Version-aware: v15-v17 use `tree_view.rng`; v18-v19 use `list_view.rng` (Odoo renamed `<tree>` → `<list>`). Errors surface as `:LintViolation` nodes linked via `:HAS_VIOLATION` edge to the View. `lint_check(language='xml')` returns RelaxNG errors alongside Python lint output. Schemas: `src/indexer/schemas/odoo_xml/` (common.rng + tree/list/search/pivot/graph/calendar/activity). Test coverage: `tests/test_relaxng_violations.py` (242 lines) + `tests/test_relaxng_violations_unit.py` (365 lines).
