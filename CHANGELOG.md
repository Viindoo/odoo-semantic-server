...
- **Persist pattern `category` end-to-end (#331):** the `patterns.json` column was a static hardcoded label only at seed time; now `category` is a real PostgreSQL column on patterns table (migration 0002) + a Neo4j PatternExample property with MERGE-key semantics. Canonical SHA -> one-time global reseed + backfill_patterns script auto-runs on first startup. `suggest_pattern` reads the live `category` property (no longer prefix-`test-` hack). Admin patterns API + UI filter by category; all 8 test + 121 production patterns now discoverable by category. Closes #331.
