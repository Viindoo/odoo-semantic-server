...
- **Pattern catalogue (WI-2):** 8 test-writing patterns seeded into patterns.json with test category: test-transaction-case, test-savepoint-case, test-http-case, test-form-case, test-computed-field, test-onchange, test-mock-model, test-domain-validation, all with "no cr.commit()" gotcha prominence. _SeedMeta sha256 update auto-reseeds changed patterns.
