...
- **A SIGTERM-ed index run could leave its job `running` (#381 F6):** the handler wrote the error on the run's main connection, which may be inside a `_write_scope` transaction and is rolled back at exit. It now writes on a fresh autocommit connection (a plain `psycopg2.connect` with a 5s `connect_timeout`, so an unreachable database never holds the exit and the handler never bootstraps the pool); `index-core` installs the same handler.
