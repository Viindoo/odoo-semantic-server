...
- **The Web UI "index core" button indexed nothing (#381 F2):** `spawn_indexer_subcommand` always appends `--job-id`, which `index-core` did not declare, so the child died on argparse exit 2 and its job stayed `queued`. `index-core` now accepts `--job-id` and reports running / done / error like `index-repo`; the spawn helper checks the real parser and refuses (no job row, nothing spawned) a subcommand that cannot report its job. A spawn that fails after the job row exists (log file or `Popen` error) marks that job `error` with the reason instead of leaving it `queued`. The index-core and seed-patterns routes answer every failure to start (refusal or spawn error) with HTTP 500 `ok: false` and the reason; they used to answer 200 `ok: true, job_id: null` ("job tracking unavailable") for a run that never started. The repo index / reset-embed / index-all 500 bodies also carry `ok: false`. A contract test parses every subcommand `src/web_ui` spawns with `--job-id 1`.
- **A Web UI index job could stay `queued` forever (#381 F1):** a child that died before its first `running` report left a row with no pid, which the start-up sweep skipped and the reset route refused (prod: jobs 3, 4, 5). The Web UI now records the child's pid right after the spawn; `mark_dead_jobs` expires a `queued` job that never started (`started_at` NULL; any pid, since a recorded pid may be reused after a reboot or belong to another user) older than `INDEXER_JOB_QUEUED_TTL_SECONDS` (new, default 600s, env override; `docs/operations/timeouts.md`); `POST /api/jobs/{id}/reset` also accepts a `queued` job. The sweep, the reset route and the `is_alive` of `GET /api/jobs/{id}/status` now share one rule (`job_registry.job_staleness`), which also expires a `running` or `queued` job whose pid is alive but is not its process: the machine booted after the job started, or that process started after the job did (pid reused, including another user's; read from Linux `/proc` with a 120 s slack for wall-clock steps, a live pid is left alone where `/proc` cannot be read). Known limit: a forward clock step over 120 s while a job runs can mark that live job's row error; a step-immune check would need a new column, deliberately not added in a patch release. Such a job used to stay `running` forever and the reset route refused it as "still alive".
- **A SIGTERM-ed index run could leave its job `running` (#381 F6):** the handler wrote the error on the run's main connection, which may be inside a `_write_scope` transaction and is rolled back at exit. It now writes on a fresh autocommit connection (a plain `psycopg2.connect` with a 5s `connect_timeout`, so an unreachable database never holds the exit and the handler never bootstraps the pool); `index-core` installs the same handler when it tracks a job (`--job-id`); without one it keeps the default SIGTERM (exit 143) as before.
- **A held orphan-embedding sweep could exit 3 with nothing actionable (#381 F3):** gate G-B's attention went only to repos of the held profiles registered at the version, so leftover rows of a profile with no repo there named nothing. The attention text names the held profile(s), falls back to every repo of the version, and the exit-3 stderr adds `embedding_orphans_held: <v>: N group(s) of profile(s) ...` (gate id unchanged). Both name the one-shot to run, `index-repo --profile <p> --allow-mass-retire`, with `<p>` a profile that has a repo at that version (a run reconciles only its own repos' versions, so the held profile itself would delete nothing), and warn that the flag lifts every mass gate of that run; `docs/deploy.md` and the rollout runbook step 5 say the same.
...
- **`test_class_inspect` no longer shows other tenants' private subclasses (#373 H6):** the subclass, twin and method hops of `build_test_class_inspect_query` had no tenant predicate, so a scoped key saw the names and the count of other tenants' private subclasses. Every child hop now applies the ADR-0034 scope predicate (framework-origin TestHelpers stay public); the count is taken after scoping.
- **Tenant members no longer receive other tenants' data from the Web UI JSON (F30, F32, F33, W1):** `GET /api/dashboard/stats` (reachable by any signed-in user) listed every tenant's profiles and repos unredacted and returned global API-key / embedding counts; `GET /api/repos/profiles` returned raw `error_msg`, `clone_error_msg`, `last_job.error_msg` (server paths, other tenants' repo URLs, stack traces) and `local_path`. Both endpoints are now tenant-scoped; one redactor (`repos._redact_repo_rows`) nulls `local_path`, `clone_error_msg` and `lifecycle_attention` and maps `error_msg` to fixed categories for non-admins; the repo delete response no longer names blocking repos of other tenants or raw exception text. Admin output is unchanged.
- **`profile_inspect` no longer names a profile outside the key's scope:** the ancestor chain, the ancestor-count label, the coverage legend, `Children`, the `[inherited from X]` tags and the repo URLs of a private ancestor/child profile of another tenant were shown; they are now disclosed as "(+N ... not visible to this key)".
- **Lifecycle answers never disclose another tenant's repos or raw errors (T1):** the `check_module_exists` / `describe_module` "Indexed: No" block printed a pending retirement's `retire_blocked_by` word for word, which for an undecidable name lists the unsynced repos (basename + repo id) of any tenant that may ship it, and for a failed retirement carries the raw Neo4j/Postgres exception text. A scoped key now sees only the reason class (`gate: <gate ids>`, `no_retire`, `undecidable`, `error`, `skipped_recent`); an admin key still sees the full reason.
- **"Renamed to" never names a module outside the caller's scope (T2):** the successor recorded for a swept orphan from a manifest `old_technical_name` was looked up across every tenant, so tenant A could be told the name of tenant B's private module that declares A's old name. The lookup is scoped to the ledger row's profile, and the read side shows a recorded `old_technical_name` successor only when a module of that name is visible to the key.
- **Web UI repo delete no longer reports (or retires) another tenant's residue by basename (T3):** the "residue" of a removed repo (Module nodes the ledger never recorded) was matched by `Module.repo` basename alone, which collides across profiles (`odoo`); the delete summary (`residue_retired`, `residue_kept`) could name another tenant's modules. Residue is now only a node owned by a removed repo's profile (or, profile-less, carrying its `repo_id`).
- **Job error categories shown to tenants match whole phrases (F34):** `sanitize_job_error` matched bare substrings ("boom" -> resource limit, any path containing `ssh` -> repository access); categories are now anchored regexes of what the producers really write. Output is still never the raw text.

### Fixed

- **A failed repo no longer hides the lifecycle outcome of the healthy ones (PR #379 review):** when one repo of a profile failed to index (for example a registered repo whose clone directory is missing), `index-repo --all` dropped that profile's presence heads and tripped gates, so its healthy repos stayed unsynced and kept holding the orphan sweep, and their gates never reached stderr; a sequential `--all` run also exited 0/3 with a failed profile. The healthy repos are now reconciled, the `Lifecycle needs attention` lines are printed for a failed run too, and a failed repo or profile always exits 1 (`IndexRunError` carries the run summary). The module-less children a missing checkout holds version-wide are now named (attention on the unsynced repo, `lifecycle-audit` `child_orphans_deferred`, counted in `child_orphans`); the rollout runbook gains a pre-flight for registered repos without a checkout.
- **Orphan embeddings are deleted under the mass gate G-B (PR #379 review):** the orphan embedding sweep had no gate of its own. More than half (and at least 20) of a version's `(module, profile)` embedding groups (or any, when the ledger shows nothing present at the version) now holds the sweep (`embedding_sweep:<gate>`, exit 3, attention on the repos of the affected profiles, bypass only with `--allow-mass-retire`); `lifecycle-audit` predicts it (`embedding_orphans_held`, counted in `embedding_orphans`).
- **Runbooks verify applied migrations by exact id:** `ORDER BY migration_id DESC LIMIT 1` sorts text ids (`m9_*` after `m13_*` after `0NNN_*`) and never showed `0003_module_presence`.
- **Renamed, deleted and moved modules leave the index on the next normal run (#378):** `check_module_exists('test_pylint', '17.0')` answered "Yes" after tvtmaaddons renamed it to `test_viin_pylint` (`0240c6b77f`), and `viin_ai_rag`, `viin_ai_agent`, `viin_ai_skill`, `viin_ai_search`, `viin_ai_pulse_memory` were ghosts inflating `impact_analysis`. Root causes: only `gc_stale_modules` deleted modules and only under `--gc`, which no job passed; the incremental path could not see a vanished module; the delete removed the Module node only. Every `index-repo` run now reconciles a per-repo lifecycle ledger and retires the Module with its whole subtree and embeddings (ADR-0056). The pre-ledger ghosts are swept by the first plain run after the deploy.
- **`test_class_inspect` phantom `[?] ?` row and inflated counts (#373):** `collect(DISTINCT {..})` after an `OPTIONAL MATCH` produced a map of nulls for every class without subclasses. Lists are now built with one-hop subqueries, deduped by (module, name), ordered, capped with "... and N more" and closed per ADR-0023; `Inherits (direct):` replaces the misleading ` -> ` chain; `module=` / `file_path=` also narrow the TestHelper fallback; the setUpClass preview discloses its cap.
- **Fields, methods, views and relations a live module no longer defines leave the graph (G4):** a re-parsed module's children and relationships that the parse did not write are pruned (run token + `written_at` guard), with their embedding rows, unless the parse was degraded, the module is shared or an unsynced repo may ship it, or the mass-drop gate holds. A shared module loses what no owner's latest complete parse defines (F49).
- **A module stamped in the run's first millisecond was retired as stale (F52, data loss):** Cypher orders two DateTimes of the same instant by zone (`Z` vs `UTC`), so the `last_seen_at >= run_started_at` race guard said "older" for a same-millisecond stamp and a concurrently protected module could be deleted. Every guard now compares `epochMillis`.
- **Web UI repo/profile delete no longer retires modules other profiles ship (F1):** `delete_modules_scoped` deleted by `Module.repo` basename, and every CE clone has basename `odoo`, so deleting one profile's CE repo retired the CE modules of every profile at that version. Removal now goes through the ledger: modules another repo still ships keep their node with ownership reset; history survives the delete.
- **Embedding deletes worked only for a superuser/BYPASSRLS owner:** under FORCEd RLS a non-bypass owner role deleted 0 embedding rows silently (ghost embeddings survived every cleanup); deletes and the orphan read now run with a transaction-local `app.allowed_profiles = '*'`.
- **The posbox `point_of_sale` stub could be indexed as `point_of_sale` in odoo12-18 (F15):** the scan now picks one deterministic winner per name (3-part version, then shallower path) and keeps the stub as a shadowed path; a node written from the wrong path is re-written by the next plain run.
- **Untracked manifests were indexed as modules (F7):** a local `.odoo-ai/` tree produced 313 phantom modules and 123 wrong paths in a local tvtmaaddons13 clone. Only git-tracked manifests are modules now.
- **A long-form manifest version decided the module's Odoo version:** `16.0.1.0.0` on a 17.0 branch was keyed 16.0. The standard branch name decides; a mismatch is flagged (`version_mismatch`, `version_raw`).
- **`cli_help` for v16 and v19 matched stale upstream text:** v16 gains `--log-config`; the v19 `--dev` help drops the removed `werkzeug` bullet (verified against the stable branches).
- **`check_module_exists` EE guard:** applied from each guard row's `since_version` only, and an indexed edition (CE / Viindoo / OCA / custom) now wins over the curated EE guard list.
- **`profile_inspect` / `module_inspect`:** a session pin no longer narrows reads of an explicitly named profile; an admin narrowing by `profile_name` sees the shared base profiles a tenant of that profile sees; trees close per ADR-0023; `module_inspect(method='tests')` reports a failed query as unavailable (not "no tests") and counts every class; `profile_name` descriptions no longer promise parent-profile content.
- **Resource cache never stores a degraded body:** `odoo://` bodies rendered while the ledger was unreachable were cached for the full TTL.
- **Test-surface graph (F2, F11/L4, F40, F41, F46):** a test class inherits only a base its module can import (no false `INHERITS_TEST` to a same-named class of an unrelated module); one run leaves the child -> helper projection edge in place; a class that stops being a helper loses its projection; an addon helper shipped by two repos is visible to both owners' tenants; `COVERS_*` edges a test no longer derives are removed.
- **OWL / core-symbol edges (F47):** an OWL component extends the parent its file imports and drops a parent it no longer names; `BOUND_TO` targets the defining Model node only; `USES_CORE_SYMBOL` binds deprecated symbols only (the `removed` arm could never match).
- **Lifecycle signals:** a skipped repo's `lifecycle_attention` states what stands now, not a past run's signal (F38); `index-repo --all` reports profiles with no repo as `profiles_empty`, not as indexed ok (F39); `lifecycle-audit` predicts self-heal re-writes on incremental and full runs too (F44); lifecycle, post-pass and pattern-sentinel reads no longer raise Neo4j unknown-property / unknown-relationship / unknown-label warnings on graphs that lack the key (F50, incl. `_SeedMeta` on a never-seeded graph); the entity prune deletes embeddings on the run's own connection (F51).
- **A named-zone datetime could crash the indexer process (SIGSEGV):** the neo4j driver packs an aware datetime by calling `tzinfo.utcoffset()` with its own `neo4j.time.DateTime`, which CPython's C `zoneinfo` misreads for a named zone (e.g. `Asia/Ho_Chi_Minh`). Every datetime sent to Neo4j now goes through one helper that converts it to the fixed UTC offset (`writer_neo4j.utc_for_neo4j`). Same driver code in 5.28.6 and 6.3.1, so no dependency change.
- **A repo that excludes a module another repo still ships leaves the module (G2):** flipping a co-owned module to `installable: False` (or license skip / unparseable) left the excluding repo's profile on the node; the reconcile now drops that owner like a co-owner's retirement (survivors re-write it; its embeddings go). A module only one repo shipped still leaves through the orphan sweep.
- **`index-repo --no-embed` removes the embedding rows of pruned entities (G1):** deleting needs no embedder; rows of entities the parse no longer produces go (matched by chunk type + entity, counted first), no new embeddings are computed.
- **`index-repo` / `lifecycle-audit` help texts** state current behaviour: `--full` is a re-parse/backfill, not a cleanup; `--no-retire` and `--allow-mass-retire` cover the entity prunes; `--fail-on-findings` lists every finding kind (G3).
- **A co-owned module is no longer re-owned while an unsynced repo may still ship it (D1):** when one repo dropped a module another synced repo still ships, the reconcile reset the node (and subtree) to the synced survivors at once, so a third repo that also ships it but had not synced yet (typical right after a deploy) lost the module from its profile until its next non-skip run. The owner drop (pending retirement, excluding co-owner, Web UI repo delete) now applies the same potential-owner rule (F48) as a retirement: undecidable (kept, attention, exit 3) for a pending name, `excluded_owner_waiting` for an excluding co-owner, re-evaluated next run.
- **A module re-written by a concurrent run during its retirement keeps its fresh children (D2):** the concurrent re-MERGE guard of `retire_modules` checked only the Module node once; a run of another profile that re-wrote the module mid-cascade lost the fields/views it had just written, while the Module survived and the ledger showed it present. Every child delete now keeps nodes written at or after the retiring run's start (`written_at`, epoch-millisecond compare), and a Module that survives the final step is reported `skipped_recent` (embeddings and ledger untouched, retried next run).
- **A tracked manifest that cannot be read no longer retires a shipped module (E2E-D1, data loss):** `parse_manifest` swallowed `OSError`, so a manifest with a permission or I/O error was classed `unparseable`; the node had no `present` row left and the orphan sweep deleted the module and its subtree (real run: `to_approvals`, 278 nodes) with exit 0 and no attention. A tracked manifest that exists but cannot be read now makes the scan incomplete (G-A `scan_incomplete`: nothing of the repo retires, exit 3, `lifecycle_attention` names the file); the module is neither present nor excluded that run. A manifest that is read but does not parse keeps an indexed module as it is (never swept, never dropped as an excluding co-owner) until it parses again, raising the soft signal `manifest_unparseable` (exit 3 + attention, the repo's other retirements proceed; `lifecycle-audit` finding `unparseable_kept`). Only a genuinely absent manifest retires a module.
- **Retired answers show the HEAD the retirement was recorded at (E2E-D2):** the reconcile never passed the repo HEAD when it recorded a row `retired`, so every retired answer read "recorded at <date> (branch HEAD not stamped yet))" (with a doubled parenthesis). The row now carries the retiring repo's HEAD, and the line reads `(recorded at HEAD <sha7> on <date>)` (or `(recorded on <date>)` for a row whose repo is gone).
- **An empty XML file no longer marks its module's parse degraded (E2E-D3):** lxml rejects a 0-byte (or whitespace-only) document ("Document is empty"), which marked the module degraded (never entity-pruned) and set `lifecycle_attention` on every run with exit 0 (real case: odoo 16.0 `mass_mailing/data/mass_mailing_data.xml`). An empty file now defines nothing and is no failure.
- **Migration 0003 re-run:** a ledger row whose `repo_id` points at a hard-deleted repo no longer aborts the FK re-add (F13).
- **Admin repos page:** loads repos from `/profiles` instead of a `GET /api/repos/repos` route that never existed (F31).
- **Unit test run:** `make test` never selects browser tests, and a browser that cannot launch skips instead of poisoning every later async test.

### Added

- **Module lifecycle ledger `module_presence` (ADR-0056):** one row per (repo, module name) with state (`present` / `excluded` / `retired`), reason, first/last seen, removing commit, successor (git rename or `old_technical_name`), resurrection count, complete-parse record; history survives repo/profile delete (FK `ON DELETE SET NULL`); RLS policy like `embeddings`. `repos` gains `presence_head_sha`, `lifecycle_attention`, `lifecycle_attention_at`.
- **`python -m src.indexer lifecycle-audit (--profile P | --all) [--version V] [--json] [--fail-on-findings]`:** dry run of the next index run (writes nothing, takes no lock, no fetch); JSON schema `osm.lifecycle-audit/3`; exit 4 on findings with `--fail-on-findings` (exit 0 otherwise). Weekly unit `docs/deploy/odoo-semantic-lifecycle-audit.{service,timer}`.
- **`index-repo --no-retire`** (incident escape hatch: scan and write, delete nothing) and **`--allow-mass-retire`** (one-shot bypass of the mass-retire / entity-prune gates, never in a timer); exit code **3** when the lifecycle needs an operator.
- **Lifecycle on the read side:** `check_module_exists` YES shows `Last seen: HEAD <sha> on <date> [repo]` (per repo when several own it) and a version note on a mismatch; `check_module_exists` / `describe_module` NO show the ledger block (state, removing commit subject, `Renamed to`), `Renamed from`, `Present at other versions` listed explicitly, the dependency-stub line, and point to the successor. Tool / resource count unchanged (31 / 9).
- **Intra-module entity and relationship prune** with per-module skips (degraded parse, shared, undecidable owner) and a soft mass-drop gate; **shared-module prune** with a bounded post-deploy bootstrap (`OSM_SHARED_PARSE_BOOTSTRAP_PER_RUN`, default 60 modules per repo per run, read on every run).
- **Web UI repo JSON:** `presence_head_sha`, `lifecycle_attention` (admin only), `lifecycle_attention_at`, `lifecycle_counts` on `/api/repos/profiles` and `clone-status`.
- **`profile_inspect` counts:** "Owned by this profile" and "Including ancestor profiles (...)" side by side; coverage shows `own`, `with_ancestors`, `indexed_elsewhere`.
- Runbook `docs/deploy/runbooks/module-lifecycle-cleanup.md`; env vars `RETIRE_LOCK_WAIT_SECONDS` (900), `WEBUI_LIFECYCLE_LOCK_WAIT_SECONDS` (20), `LIFECYCLE_AUDIT_QUERY_TIMEOUT_SECONDS` (120), `OSM_SHARED_PARSE_BOOTSTRAP_PER_RUN` (60) - `docs/operations/timeouts.md`.

### Changed

- **Retirement is unconditional:** every `index-repo` run retires what no repo ships any more, under gates G-A (scan complete and trusted) and G-B (more than 50% and at least 20 modules, or all of them). A tripped gate deletes nothing, sets `repos.lifecycle_attention` and exits 3.
- **G-B also guards the first run after the deploy (D3, owner decision):** while the ledger has never reflected a repo (`presence_head_sha` NULL: the first runs after the deploy, a new registration) its mass gate compares the scan with the Module nodes the GRAPH attributes to the repo (`repo_id`, or its profile + basename), per `(version, name)`. A scan that no longer covers more than half (and at least 20) of them - untracked or gitignored manifests, a broken checkout, a mass re-key to the branch version - trips `mass_retire`: the repo stays unsynced, the orphan sweep keeps every orphan of its profile, exit 3 + attention, until a confirmed `--allow-mass-retire` run. Normal ghost cleanup of the other repos proceeds. `lifecycle-audit` predicts the same (`gates.baseline = "graph"`).
- **`--gc` is deprecated and has no effect** (still accepted; logs a warning). The Web UI no longer offers a GC option; its API still accepts `gc` and logs it as deprecated (G4). `--full` is no longer needed for cleanup; it remains the way to backfill a new index-time property, and now also entity-prunes every module it re-parses.
- **A run where nothing changed skips the version-wide post-pass (E2E-D4):** the post-pass (same-name INHERITS, OWL edges, test-surface edges and COVERS_*) re-derived every edge of the version on every run, about 22 s for odoo17 + tvtmaaddons17 even when every repo took the unchanged skip. A `PostPassState` node per version is marked dirty before any repo write and by every lifecycle delete, re-own and `index-core`, and is stamped clean with a digest of the deriving code; a clean version is skipped (no-change run 23.2 s -> 0.7 s; a forced post-pass afterwards changed no edge). A deploy that changes the derivation re-runs it once.
- **The unchanged skip** additionally requires `presence_head_sha == HEAD` and no `needs_rewrite` module; otherwise a repo takes the sync path (scan + ledger, no re-parse of unchanged modules). The first run after the deploy is a sync run for every repo.
- **Version rule:** the standard branch name (`X.Y`, `saas-X.Y`) is the module version; a non-standard branch falls back to the profile version; disagreements raise `lifecycle_attention`.
- **Existence on the read side is `size(profile) > 0`:** a profile-less dependency stub answers "No" to every caller, admin included.
- **Web UI delete/rename** waits at most `WEBUI_LIFECYCLE_LOCK_WAIT_SECONDS` for the git and ledger locks, else HTTP 409 with `Retry-After` and nothing changed.
- `describe_module` always ends with a `Next:` footer; `test_class_inspect` prints `Inherits (direct):`.
- Removed `Neo4jWriter.gc_stale_modules` and `delete_modules_scoped` (replaced by `retire_modules` / `drop_module_owner` and the ledger reconcile). `ops/cleanup_noninstallable_stale_modules.py` is deprecated (kept one release).

### Migration

- **`0003_module_presence`**: run `python -m src.db.migrate`, then `ops/regrant_osm_reader_after_migration.sh` (SELECT on `module_presence` for `osm_reader`). New Neo4j indexes are created by `setup_indexes()` at the start of the next index run.
- **No manual cleanup:** the first normal `index-repo --all` (no `--full`, no `--gc`) syncs every repo and removes the ghost modules. Before deploying, run the read-only F24 check (Module nodes with a real path and an empty profile read "No" after the deploy) and follow `docs/deploy/runbooks/module-lifecycle-cleanup.md`.
- **Enable `odoo-semantic-lifecycle-audit.timer` only after the shared-module bootstrap drained** (every audit finding 0; about 11 nightly runs for a CE clone): it runs `--fail-on-findings` and would alert weekly on the backlog.
- **`cli_help` v16/v19 corrections** are curated CLI data written by `index-core`: re-run `python -m src.indexer index-core --source <odoo checkout> --version 16.0` (and `19.0`) to apply them.
- **Development databases only:** `0003_module_presence` was edited on the branch after it was first applied (last edit: `last_full_parse_*`). yoyo applies a migration once by id, so a dev/staging database migrated from an earlier branch commit fails with `UndefinedColumn`; apply `migrations/0003_module_presence.rollback.sql`, delete its `_yoyo_migration` row and migrate again. No released database ran an earlier 0003.
- Never put `--allow-mass-retire` in a timer or drop-in. Rollback: redeploy 0.18.x - its Web UI returns repo rows unredacted, so clear `repos.lifecycle_attention` first (it can name other tenants' repos), and rolling back re-opens the tenant leaks fixed here; a module retired by mistake comes back with `index-repo --full` of its owner's profile.

### Also in this release (merged to master before #373/#378)


- **Backfill now prunes curated patterns removed from `patterns.json`, preserving admin-created rows (R1 sibling, Postgres layer):** `ops/backfill_patterns.py` syncs the curated `patterns.json` SSOT into the Postgres `patterns` table (the reseed source-of-truth read by `seed_patterns.run()`), but was UPSERT-ONLY (`INSERT ... ON CONFLICT DO UPDATE`) with no delete - so a curated `pattern_id` renamed/removed in `patterns.json` (e.g. #362's `owl2-component-v15` -> `owl1-component-v15`) left the OLD row in the DB forever, and because the DB is the reseed source it re-propagated the stale id to Neo4j/pgvector on every reseed (the R1 Neo4j prune could not clean it, since the DB still listed the stale id as live). Same orphan-on-rename class as R1/F2, one layer down. Fix: after the upsert, `_prune_curated_removed` soft-deletes (`soft_deleted = TRUE`, recoverable; the reseed reads `WHERE soft_deleted = FALSE`) curated rows whose `pattern_id` is no longer in `patterns.json`. **Admin-safety (the #1 requirement):** the `patterns` table also holds admin-created rows (net-new ids via the web UI), so the prune is SCOPED to `updated_by IS NULL` - backfill never sets `updated_by` (curated rows are NULL); admin create/update/soft-delete always set it (NOT NULL) - and therefore NEVER soft-deletes an admin-created or admin-edited row. EMPTY-GUARD: an empty json id set prunes nothing (a failed/empty load can never wipe the catalogue), mirroring the R1/F2 guards. Prune is default-on (backfill IS the SSOT sync) with a `--no-prune` escape hatch for a partial/hand-edited json; a safety log names every soft-deleted id. `backfill()` now returns `(inserted, updated, pruned)`. New postgres integration tests in `tests/test_backfill_patterns.py` (curated rename pruned; **admin row preserved**; empty-json prunes nothing; owl2->owl1 transition; idempotent). No schema migration. KNOWN follow-ups: an admin-EDITED curated pattern later removed from json lingers (deliberate - the `updated_by` discriminator cannot tell admin-created from admin-edited; the safe default is to never touch an admin-touched row); and a remove-then-re-add of a curated id does not auto-resurrect the soft-deleted row (fast-follow, same discriminator).
- **`delete-webui-user` CLI now blocks deletion of a user who owns live pattern rows (adversarial review of the above, data-loss closure):** the admin-safety scoping above relies on `updated_by IS NULL` meaning "curated, backfill-owned" forever once an admin sets it NOT NULL - but `patterns.updated_by REFERENCES webui_users(id) ON DELETE SET NULL`, and `python -m src.manager delete-webui-user` (`src/db/auth/_user.py::delete_user`, a real hard DELETE, not the GDPR `purge_user_account` tombstone path) is a production-reachable command. Deleting a user who had created or edited a pattern would silently flip that pattern's `updated_by` back to NULL, so the VERY NEXT `ops/backfill_patterns.py` run would misclassify it as curated and soft-delete it if its `pattern_id` is absent from `patterns.json` - a genuine admin-data-loss path the prune's own safety invariant did not anticipate. Fix: `_cmd_delete_webui_user` now queries `patterns WHERE updated_by = <user> AND soft_deleted = FALSE` before deleting and refuses (exit 1, listing the affected `pattern_id`s, no bypass flag) until those rows are re-attributed (any admin `PATCH /api/admin/patterns/{id}` re-sets `updated_by`) or explicitly soft-deleted. New tests in `tests/test_manager_cli_delete.py` (RED on the unguarded CLI: the owning user's delete succeeded and orphaned the pattern's `updated_by`; GREEN once the guard blocks it; a control test confirms deletion still succeeds once the pattern is soft-deleted first). No schema migration.
- **Orphaned `PatternExample` node on `pattern_id` rename now pruned (R1, issue #362 follow-up):** a `PatternExample` is MERGE-keyed on `pattern_id` ALONE, and the Neo4j writer `write_pattern_examples` was MERGE-only with NO prune step, so renaming a pattern_id in `src/data/patterns.json` left the OLD id's node in the graph forever - reachable via the `odoo://{version}/pattern/{pattern_id}` resource (direct-id fetch), serving stale content. This is exactly the gap the #364 `owl2-component-v15` -> `owl1-component-v15` rename exposed: pgvector is already a clean DELETE-then-INSERT (`seed_patterns._write_pgvector_with_embedder`) so `suggest_pattern` semantic search was clean, but the Neo4j direct-id path was not. The writer now takes a `prune` flag and, after MERGE-ing the incoming catalogue, DETACH DELETEs every `PatternExample` whose `pattern_id` is NOT in the incoming list - mirroring `prune_framework_test_helpers` (the #362 TestHelper prune of the same shape) for style, logging and idempotence, and put INSIDE the writer (the SSOT) so both reseed paths inherit it. **Defensive contract:** because the prune is necessarily GLOBAL (PatternExample carries no version/profile/repo in its identity), `prune=True` is passed ONLY on the FULL-catalogue write path - `seed_patterns.run()` prunes only when `odoo_version_min_filter is None` and the `seed-patterns` CLI only when `--version` is unset - because a version-filtered run (`_load_patterns` keeps only entries whose `odoo_version_min` matches) is a PARTIAL batch, and a global prune there would wipe every other version's live patterns. An empty incoming list never prunes (early return), so a transient empty load can never wipe the catalogue. **Ops:** cleaned on the next full pattern reseed (the nightly `index-repo` auto-reseed via the `_SeedMeta` sentinel, or `python -m src.indexer.seed_patterns` with no `--version`); no schema migration (the `pattern_id` MERGE key is untouched). Tool/resource surface unchanged (31 tools / 9 resources). New integration test `tests/test_writer_patterns_prune.py`; `IndexWriterProtocol.write_pattern_examples` signature + docstring updated. ADR-0054 (prune pattern precedent); ADR-0003 (PatternExample storage).
- **Admin pattern CRUD now INVALIDATES the reseed sentinel instead of stamping it (F1, PR #368):** an admin create/update/soft-delete of a pattern via the web UI commits only to the Postgres `patterns` table; the same request then called `_bump_sentinel()` -> `recompute_sentinel_sha()`, which stamped the `patterns_neo4j`/`patterns_pgvector` `_SeedMeta` sentinels to the CURRENT post-write DB-content SHA - before any Neo4j/pgvector write. Because `seed_patterns.run()`'s reseed gate recomputes the identical SHA from the identical DB rows, it then saw `current == stored` and SKIPPED, so the admin's edit NEVER reached Neo4j `PatternExample` nodes or pgvector `embeddings` (the endpoint's own `"reseed_status": "pending"` response was aspirational, not honored). Introduced in `621616b` (the DB-primary SHA unification that fixed a prior *perpetual*-reseed bug over-corrected into this *never*-reseed bug); violated ADR-0007 D6 ("sentinel stamped only AFTER a successful seed"). Fix: the CRUD path and the manual `POST /sentinel/recompute` endpoint now call a new `invalidate_patterns_sentinel()` that CLEARS all three sentinel keys (`patterns_neo4j`, `patterns_pgvector`, and the legacy `patterns` fallback), so the next `index_profile()` reseed sees drift and propagates; `run()`'s stamp-after-successful-seed is untouched (no perpetual reseed). Best-effort when Neo4j is down (the DB write already committed; a later reseed self-heals) so a CRUD request never fails on Neo4j reachability. `recompute_sentinel_sha()` is retained for the stamp-after-a-real-seed/diagnostic case with a loud do-not-call-from-CRUD warning. New un-mocked integration test `tests/test_admin_patterns_propagation.py` asserts the edit reaches Neo4j after the next reseed; the pre-existing `test_crud_then_run_then_skip_cycle` (which had MASKED the bug by hand-forging a stale sentinel) was rewritten to assert the real behavior. ADR-0007 D6-CRUD (new note).
- **Version-scoped prune for LintRule/CLICommand/CLIFlag on `index-core` (F2, PR #368):** the same MERGE-only orphan-on-rename gap fixed for `PatternExample` (R1) existed in the three version-keyed spec writers - `write_lint_rules`/`write_cli_commands`/`write_cli_flags` never deleted a removed id. ACTIVE instance: #364 dropped lint rule `W8140` from v14-v19 (SQL-injection duplicate consolidated onto upstream `E8501`), so a re-index would leave stale `W8140` `LintRule` nodes in six versions forever. Fix: three version-scoped prune methods (shared `_prune_versioned_spec_nodes`) wired into `index_core` after each `write_*`, deleting nodes at that `odoo_version` whose id is not in the run's full live set. Version-scoped by construction (a prune for one version can never touch another). Two mandatory guards: an EMPTY-GUARD (empty live set never deletes - a degraded parse can't wipe a version) and a SOFT-DROP GATE (skip + WARNING if the delete would exceed 50% of the version's existing nodes, mirroring `gc_stale_modules` and ADR-0005's ">20% drop = suspect path refactor"). `index_core` is the sole caller and always writes the full per-version set, so no `prune=` flag is needed (unconditional call, mirroring `prune_framework_test_helpers`). **CoreSymbol (incl. tools_symbols) is deliberately EXEMPT** - its cross-version lifecycle (`added_in`/`removed_in`/`deprecated_in` + `REPLACED_BY`) is consumed by `api_version_diff`/`find_deprecated_usage`/`lookup_core_api`, and a regression test (`test_no_prune_core_symbols_method_exists`) guards that no `prune_core_symbols` is ever added. New version-scoped + wiring tests. ADR-0055 amended.
- **One-time cleanup of stale WRONG `CoreSymbol` nodes from #364 tools_symbols corrections (`ops/cleanup_stale_tools_symbols_364.cypher`, PR #368):** because CoreSymbol is prune-exempt (above), #364's same-version curated-data *corrections* (not lifecycle removals) leave stale wrong nodes a prior `index-core` already wrote: `odoo.tools.pycompat` @ v8.0/v9.0/v10.0 (the module does not exist before v11) and the flat `odoo.tools.image_process` @ v19.0 (re-export dropped from `odoo/tools/__init__.py` at v19; the real path is `odoo.tools.image.image_process`). A one-time, version-precise, idempotent Cypher (4 targeted `DETACH DELETE`s, verified against the `spec_data` diff) removes exactly those; it is explicitly NOT a standing prune (that would destroy CoreSymbol history). Run once during the deploy re-index phase after `index-core` re-runs for the affected versions.
- **Curated version-keyed data audited and corrected across cli_flags, lint_rules, tools_symbols, and `_DEPRECATED_API_SYMBOLS` (issue #364, ADR-0055):** #364 was filed as a PRECAUTION - "hand-curated, version-keyed data with no mechanical oracle" - explicitly NOT claiming a defect existed. An audit across all twelve real checkouts (v8.0-v19.0) proved the risk real: every family had source-verified errors. **The headline finding:** the same real Odoo rename, `track_visibility` -> `tracking`, was wrong in TWO independently-maintained curated artifacts, each carrying a DIFFERENT wrong value - `_DEPRECATED_API_SYMBOLS` (`src/indexer/parser_python.py`) claimed v17, `lint_rules` rule W8167 claimed v16; real Odoo source (grep across checkouts, 0 hits of `tracking=` before v13 vs 51 at v13) puts the transition at v12 -> v13. Two curated facts disagreeing with each other AND with source, on the exact same rename - the duplicate-SSOT failure ADR-0054 exists to end, caught in the wild. **Two independent gaps were stacked on top of each other:** (1) the repo's live-parser drift guards had NEVER RUN anywhere, including on a box with all twelve checkouts on disk - they discovered source via `ODOO17_SRC` etc. defaulting to `/nonexistent/odoo17`, a rival convention to the newer `OSM_ODOO_CHECKOUTS` (`tests/_odoo_checkouts.py` now unifies discovery; legacy per-version env vars are still honoured so no existing setup breaks); (2) waking them was NOT enough - those tests only asserted a flag EXISTS, never that its VALUE was right, so they went from SKIP to PASS while the underlying data stayed wrong, until new content-parity tests were added that compare field VALUES against a live parse, not just presence.

  **Corrections by family:** `cli_flags` - 967 field corrections (441 `default`, 234 `help`, 229 `type`, 63 `status`) across all 12 versions, REGENERATED from the production oracle (deliberately narrow - may only correct an oracle-covered field on an entry the oracle actually saw; never deletes an entry, adds a field, or nulls a curated-only one). Includes `--load`'s default truncated at the first comma, ~15-19 fabricated `type` values per version, help text bled onto neighbouring flags (present even in the freshly-redone v19 file), and `--debug` wrongly marked deprecated toward `--dev` at v8/v9 - a flag that does not exist at v8, a v10+ fact bled BACKWARD. `lint_rules` - W8202 `name_get` carried THREE mutually inconsistent removal claims across v16/v17/v19 (v16 recommended the replacement a version early; v17 said "removed in v17", false - still defined there, deprecated only; v19 said "removed in v17+", contradicting v18's own correct entry); ground truth is deprecated v17, removed v18. W8167 `track_visibility` -> `tracking` corrected v16 -> v13 (see headline). `tools_symbols` - `pycompat` no longer claimed present at v8/v9/v10 (the module does not exist until v11; OMITTED per the schema's own "removed = absent" convention, not flagged "removed"); `image_process`'s signature, frozen and copy-pasted identically across all 7 versions it appears in, corrected per version (`base64_source` v13-v15, `source` v16+, `expand`/`padding` added v18), its fabricated `-> bytes` return annotation dropped (the real function returns base64 text), and the v19 flat `odoo.tools.image_process` is now OMITTED rather than "deprecated" (it raises `ImportError` at v19 - a different thing from "available but discouraged"; `html_escape` stays correctly "deprecated" at v19 as the contrast case, since it genuinely still resolves). `_DEPRECATED_API_SYMBOLS` (`parser_python.py`) - all 25 entries audited; 8 were wrong (`track_visibility` v17->v13, `oldname` v15->v14, `default_get` v17->v19, `html_escape` v17->v15, `_search` conflated a v18 fact with a v19 one), THREE were FABRICATED outright (`float_compare`, `float_round`, `get_modules` claimed deprecations that never happened - their imports are unchanged v8-v19), and `fields_get` is now recorded UNVERIFIED rather than guessed (no semantics change is findable v14-v19 or in git history). The claims now live in a `DeprecatedApiSymbol` dataclass with a CI-pinned parity test, not in a trailing comment - a version fact inside a comment cannot be tested, which is why it rotted. File-level `_note` prose across all 38 curated files was swept too, since the array-only corrections above left the SAME file asserting a corrected fact in one place and stale folklore in another: `@api.multi` removed at v13 not v14; OWL first appears at v14, not v13; OWL 2 (2.8.2) lands at v16, not v15 (v15 is still OWL 1.4.11); OWL is NOT at v3 anywhere through v19 (still 2.8.2); `spreadsheet` present from v12 not v13; `active_test` from v9/v10 not v13; the `openerp` -> `odoo` namespace rename completes at v10, not v9 (v9's `odoo/` dir holds zero git-tracked files). `group_operator` -> `aggregator` at v18 was re-checked on suspicion and found CORRECT (no change needed).

  **Three silent-swallow bugs of the same shape**, each an oracle that quietly stopped being an oracle with zero signal: `parser_cli.py`'s AST walk died on `os.chmod(self.rcfile, 0600)` (a Python-2 octal) at v8, v9 AND v10 - Odoo itself only fixed this to `0o600` at the v10->v11 boundary, ONE VERSION LATER than the codebase's own `openerp`/`odoo` namespace split, so the CLI oracle silently returned zero flags for three whole versions; recovered via the same AST-first/text-regex-on-`SyntaxError` fallback shape ADR-0054 established (v8/v9/v10 now recover 72/66/67 flags respectively; v11-v19 stay byte-identical). Real Odoo v19's `test_lint` `.eslintrc` has a trailing comma (valid JSON5, invalid strict JSON) - `json.loads` raised, the exception was swallowed, and ALL v19 eslint rules were silently dropped in production with zero signal; a narrow lenient loader recovers 13 rules, with a regression test that fails if the real file ever stops needing it. And a live PRODUCTION bug, not just curation drift: `parser_odoo_core.py` emits `odoo.tools.sql.SQL` while curated `tools_symbols` data used the flat `odoo.tools.SQL`, so the string-equality dedup in `pipeline.py` never fired for it and `lookup_core_api("SQL", ...)` deterministically returned the THINNER curated node instead of the fuller parsed one - fixed as a CLASS (any parsed `odoo.tools.<submodule>.<name>` is now also indexed under its flattened alias for dedup), not as a one-off patch for `SQL` alone.

  **Governance - the minimum bar (new ADR-0055):** every curated artifact added to this repo from now on needs ONE of an oracle, a recorded reason it cannot have one, or provenance metadata. Concrete instances landed in this pass: 19 of 69 distinct curated `pylint-odoo` rule_ids collide with a real, currently-assigned OCA code carrying a DIFFERENT meaning (curated `W8110` = "`_columns` deprecated" vs real `W8110` = "missing return after `super()`") - every `lint_rules_*.json` record (603 total across 12 versions) now carries required `rule_id_source` (`"upstream"`/`"osm-local"`) plus `rule_id_collision`; ids were NOT renumbered (`rule_id` is a Neo4j MERGE-key component, externally visible via `lint_check`). `W8140` (OSM-local) and `E8501` (real, Odoo-vendored `_odoo_checker_sql_injection.py`) carried an identical SQL-injection regex at v17-v19, so `lint_check` double-reported one defect - and a test was previously GUARDING that duplication as expected behavior; consolidated onto the upstream `E8501` id from v14 (the same boundary the live gate uses), `W8140` kept only v8-v13 where no live `E8501` oracle is wired. `tools_symbol.schema.json`'s `note` field claimed "shown in tool output" for 204 curated entries that nothing in the load/write/MCP chain read - write (`writer_neo4j_spec.py`) and render (`lookup_core_api`) are now wired; the one remaining loader hop is deliberately left unwired (a reserved file held by concurrent work) and PINNED by a test that will fail the moment the gap should close, rather than left silently latent. `cli_flag.schema.json` was loaded but never actually validated (`_load_schema()` was dead code) - all three families' schemas are now validated by the real `jsonschema` library, each proven capable of FAILING on a deliberately broken record, which caught 11 pre-existing violations (integer `default` where the schema declares string) spanning `cli_flags_12.0.json` through `cli_flags_18.0.json`.

  **Also widened and newly built:** the `lint_rules` live-extraction gate (`LINT_RULES_MIN_MAJOR`) moved 17 -> 14, but deliberately NOT to 13 - v13 has TWO real checker files, one matching the extraction glob (`E8501`) and one not (`E3110`); a partial extraction that silently under-reports was judged worse than an honest exclusion, so v11-v13 stay out with the reason recorded in the constant's own comment (alongside v8-v10, which have no checker source at all). Live coverage moves 3/12 -> 6/12 versions. A `tools_symbols` oracle (`parser_tools_symbols.py`) was built from scratch, mirroring the ADR-0054 AST-first/text-fallback shape: `curated-only` is ZERO at all twelve versions (every curated symbol is found in real source); `oracle-only` is large (600-1500 names per version) and is NOT a defect (the curated set is a deliberately small hand-picked subset; `odoo.tools` genuinely re-exports hundreds of names, including stdlib names leaking through `import *` on files without `__all__`). Two recovery modes were needed: `html_escape` (a module-scope `if`/`else` def at v8-v13, becoming a plain assignment from v15+ - recovered by descending into `If` bodies and tracking every non-underscore top-level `Assign` target) and `ustr` (never a def/class/assign at all - bound in `misc.py` via `from odoo.loglevels import ustr` and re-exported by `from .misc import *` - recovered because `ImportFrom` targets now contribute to a module's public surface).

  **Operators:** run `index-core --source <checkout> --version <X.0>` per version to pick up every correction above in an already-deployed graph - these are all `spec_data/*.json` curated facts written by the `index-core` pass, not the ordinary per-profile reindex. No special urgency: nothing here is a security fix, and every finding was a wrong or missing FACT, not a crash. **Going forward:** the two dormant guard classes now run on every CI pass and on any dev box with `OSM_ODOO_CHECKOUTS` set (default `~/git`), and the new content-parity tests catch a future VALUE drift, not just a missing entry. No version bump; tool/resource surface unchanged (31 tools / 9 resources); no schema migration (the new `rule_id_source`/`rule_id_collision` properties are additive on the existing `LintRule` MERGE key). ADR-0055 (new); ADR-0054 amended.

- **`test_base_classes` is now genuinely per-version (issue #362, ADR-0054):** the framework test-base-class menu (TransactionCase, SavepointCase, HttpCase, Form, ...) returned the IDENTICAL 11-entry list at every indexed Odoo version, v8 through v19, because `seed_framework_helpers(odoo_version)` used its version argument only to stamp the written node and then wrote all 11 entries of a flat, version-blind `_FRAMEWORK_BASES` dict unchanged under every version - correct Cypher and version resolution over uniformly wrong data. Concrete wrong strings this fixes: `SavepointCase` shown as `"(deprecated alias, v8-v15)"` at every version incl. v19 (real: present v8-v16, deprecated from v15 via a genuine `__init_subclass__`/`warnings.warn`, REMOVED at v17 - it was never an alias); `TreeCase` shown as `"(v14+)"` (real window v11-v14 - inverted, pointing agents at a class absent from v15-v19); `BaseCase` shown as `"v10+"` (real: v8+); `HttpCaseCommon` seeded at all 12 versions (real: v14 ONLY); `Form`/`O2MForm` shown as `"v14+"` (real: v12+, relocating to `odoo/tests/form.py` at v17). A second, independent copy (`_static_framework_bases` in `src/mcp/tools/test_tools.py`, the empty-graph fallback) carried only 4-5 classes and disagreed with the graph's 11 at nearly every version - both copies are now deleted. New single source of truth `src/indexer/framework_bases.py`: one feature-owned `VersionRegistry` with 7 eras per ADR-0052, plus an independent AST-oracle/text-regex-fallback parity test (`tests/test_framework_bases_parity.py`) that fails CI the moment the curated table drifts from a real parse - this is what makes it parse-VERIFIED curation, not "a curated table again". `framework_bases(v)` is now the READ-SIDE AUTHORITY for both the writer and `test_base_classes` (the graph is consulted only to enrich `file_path`/`line` by name-join, never to decide which classes appear) - **the menu and the `name=` drill-down are correct at DEPLOY, on every already-indexed server, before any reindex runs.** The drill-down also no longer falls back to printing the whole menu on a miss - it now states the real window, deprecation point, removal point, and replacement for the queried name. **A reindex IS still needed to clean up the graph itself:** before this fix nothing in `src/` could ever delete a `TestHelper` node (the writer was MERGE-only; `gc_stale_test_nodes` covers `TestClass`/`TestMethod` only; `delete_module_subtree`'s cascade excludes `TestHelper`), so a class removed from an era (e.g. `SavepointCase` at v17+) would sit in the graph forever. The writer gains `prune_framework_test_helpers`, wired into the version-wide `reconcile_test_surface` post-pass that runs on every profile-index pass - operator command: `index-repo --profile <name>` or `index-repo --all` (the existing nightly cron already does this; `--full` is not required for the prune specifically, though it remains the standard periodic hygiene pass for other stale-node classes). Verify: `MATCH (h:TestHelper {module:'@framework', odoo_version:'17.0'}) RETURN h.name ORDER BY h.name` should return exactly 7 names with no `SavepointCase`. Tool/resource surface unchanged at 31 tools / 9 resources; no schema migration (`TestHelper` MERGE key `(name, module, odoo_version)` untouched; only the non-key `file_path`/`line` properties move from always-null to sometimes-populated, coalesce-ON-MATCH so a source-less reseed never wipes prior enrichment). Also fixes the identical folklore in `src/data/patterns.json`'s `test-savepointcase-v8-v15` pattern (743b7d4) and two pre-existing ADR-0023 tree-grammar defects in the rewritten formatter. ADR-0054; ADR-0051 amended.
- **Version-scope cross-repo dependency propagation (ADR-0007 W14 over-reset fix):** the incremental reindex's cross-repo dep propagation re-indexed and re-embedded FAR more repos than needed. `src.indexer.cross_repo.find_dependent_repos` already scopes its Neo4j `DEPENDS_ON` query to one `odoo_version` and returns bare repo BASENAMES, but `RepoStore.get_repo_ids_by_local_path_basenames` (`src/db/repo_registry.py`) matched those basenames against `repos.local_path` with NO version filter - so a single upstream change at one version reset the SAME repo cloned in every version's profile (e.g. `tvtmaaddons` spans 12 profiles v8-v19 -> reset 12x). On the first post-fetch catch-up run this ballooned the embedding backlog into a 10h+ slog. Fix: thread the already-in-scope `odoo_version` into `get_repo_ids_by_local_path_basenames` and add `JOIN profiles p ON r.profile_id = p.id AND p.odoo_version = %s`. This only NARROWS the cross-VERSION over-reset (verified: zero genuine same-version basename collisions exist today); it preserves real cross-profile SAME-version propagation (e.g. `viindoo_internal_17`'s `saas-infrastructure` DEPENDS_ON `standard_viindoo_17`'s `tvtmaaddons`), so no legitimate reset is dropped. The "safe, over-eager" same-version-collision fallback + WARNING and the `--full`/`_is_incremental` gate are untouched. Schema-free, no migration, no re-index required (purely forward-looking on future incremental runs). ADR-0007 W14; MED-3 tenant-blindness stays separately deferred. (ADR-0007 + INDEX.md amended to reflect the fix.)
- **Nightly reindex now fetches upstream before scanning (fetch-gap fix):** the scheduled reindex (`index-repo --all`, systemd timer 03:30) previously only read each clone's LOCAL git state (`rev-parse HEAD` / `merge-base` / `diff` in `incremental.py`) and never ran `git fetch`, so when an upstream branch advanced (a merged PR) local HEAD still equalled `repos.head_sha` and the repo was silently skipped - upstream merges were structurally invisible to the cron (only the on-demand web `clone-all` path ever refreshed a clone; ADR-0035 documented "a cloned repo is never refreshed in-place"). `_index_repo` now calls a new `refresh_before_scan()` FIRST, reusing `src.git_utils.refresh_repo` (`git fetch --prune` + `reset --hard origin/<branch>`, ADR-0035 SSH hardening: pinned known_hosts, StrictHostKeyChecking=yes, 0o600 tempfile key) under the SAME per-repo advisory lock the cloner uses (`_repo_git_lock`, ADR-0035 D2), so a scheduled fetch and a concurrent `clone-all` for one repo serialize instead of racing on `.git/index.lock`. On by default; opt out per run with `--no-fetch`. **Admin-triggered reindex in the web UI (per-profile reindex, reset-and-reindex, and index-all routes in `src/web_ui/routes/repos_indexing.py`) now ALSO refreshes each clone (`git fetch` + `reset --hard origin/<branch>`) by default, the same as the nightly cron** - this is intended so a manual reindex picks up merged upstream PRs too; the CLI `--no-fetch` flag opts out for on-disk-only runs (the web routes keep fetch on). FAIL-SAFE: any fetch/SSH/network error is logged and swallowed (index the on-disk state) so network reachability never becomes a hard dependency of the nightly job; benign lock contention logs at INFO, genuine fetch failures at WARNING. SSH keys are resolved through ONE shared leaf SSOT `src.ssh_key_resolve.resolve_ssh_key_pem` (used by BOTH `refresh_before_scan` and the on-demand cloner, so they cannot diverge): it decides by URL scheme (`is_ssh_url`), decrypts via a new `src.crypto.decrypt_private_key` (the web `ssh_keys.py` wrapper now delegates to it), and raises `SshKeyUnavailable` for an SSH-scheme repo with no usable key - which is SURFACED (WARNING + skip fetch, index on-disk) rather than run as a doomed keyless SSH fetch that would silently leave the clone stale. `src/indexer` never imports `src.web_ui` (the resolver's deps are `src.git_utils` + `src.db` + `src.crypto`, none of them web) - enforced by a new import-discipline test. No schema/migration, no tool/resource surface change (still 31 tools / 9 resources).
- **Reindex unit now provisioned with FERNET_KEY (completes the fetch-gap fix above):** `odoo-semantic-reindex.service` gains `LoadCredential=FERNET_KEY:/etc/credstore/FERNET_KEY`, the same systemd credential-store delivery already used by `odoo-semantic-webui.service` and `odoo-semantic-backup.service` (ADR-0020). Without it, the fetch-before-scan step added above was inert in production: every indexed repo is SSH, and fetching an SSH repo requires decrypting its stored key via `src.crypto`, which requires FERNET_KEY - the unattended nightly timer runs as `odoo-semantic` (not root), so it cannot use the root-only `osm-fernet-run` wrapper the way ad-hoc CLI runs do. Config-only change; apply with `systemctl daemon-reload` (oneshot unit, no restart needed). **Sibling gap closed the same way:** `docs/deploy.md` §3.6's monthly `--full --gc` cleanup command (also reaches `refresh_before_scan` via `index-repo --all --full --gc`, same as the nightly timer) previously used a bare `systemd-run` invocation with no `LoadCredential=`, silently hitting the identical fetch-fails-fall-back-to-on-disk gap on every manual monthly run; the doc now uses the existing `osm-fernet-run` wrapper (`docs/deploy/osm-fernet-run`, already used for ad-hoc backup/restore/rotate-fernet CLI runs), which already wraps `-p LoadCredential=FERNET_KEY:/etc/credstore/FERNET_KEY`. Docs-only, no code change for this part. Audited every other OSM systemd unit/timer (`index-core`, `reembed-stubs`, `audit-repo`, `seed-patterns`, backup, TTL cleanup, alert) - none of them call `refresh_before_scan`, so no further FERNET gaps found; the webui-spawned reindex subprocess already inherits `$CREDENTIALS_DIRECTORY` from its own `LoadCredential`-provisioned parent unit. **Doc sweep for the same gap:** synced every other stale FERNET-consumer reference found across `docs/` to the 3-consumer fact (webui, backup, reindex) - `docs/deploy.md` (§12 resolution order, Option B enumeration, credstore-rollout restart list, and the indexer-needs-FERNET/MCP-does-not line), `docs/adr/0020-fernet-key-delivery.md` (provisioning-order line + hard-fail note), `docs/deploy/install-runbook.md`, `docs/deploy/reindex-v8-v19-runbook.md`, `docs/deploy/disaster-recovery.md`, and `docs/deploy/runbooks/fernet-provision.md` (previously scoped to webui+backup only, now covers all 3 including the oneshot-unit nuance that reindex needs `daemon-reload`, not a `restart`, since it has no persistent process).
...
- **Index a module identity card (#121 P2):** the human-authored display name (`shortdesc` = `manifest['name']`, e.g. "E-Invoice - VNIs VN-Invoice Integrator") and raw `author` (`manifest['author']`) are now indexed on the Module node ALONGSIDE the normalized `copyright_owner` (ADR-0036, unchanged). `check_module_exists` renders an "Identity (from indexed manifest):" block and `describe_module` shows the display name as the top header line plus the author in the Manifest sub-tree (both rendered only when non-NULL - graceful degrade before backfill). The server exposes RAW data + a provenance label and does NOT classify brand/provider; the display name surfaces the provider directly, fixing the issue #121 factual errors (VNIs read as "VNPT"; MISA meInvoice omitted) at the source. New shared `_normalize_author()` helper (str|list -> str|None), DRY with `_derive_copyright_owner`. ADR-0053.
- **Edition reclassification + verbose 3-tier labels (#121 P5):** `_detect_module_edition` now classifies an OPL-1 module authored by Viindoo/TVTMA (e.g. `l10n_vn_viin_accounting_meinvoice`, which lacks the `viin_`/`to_` prefix) as `viindoo` instead of `custom` - placed after the OEEL-1 check so the ADR-0036 invariant "OPL-1 != enterprise" is preserved (0 false-negatives on 743 real Viindoo modules). Edition labels are sharpened to carry free/paid/not-resold semantics ("Community (CE) - free, bundled with Odoo CE" / "Viindoo Commercial - paid Viindoo subscription app" / "Odoo Enterprise (EE) - Odoo S.A. licensed, not resold by Viindoo"); the #263 guard (a `viindoo` module never reads "Odoo Enterprise") is preserved. INTENDED: the custom(4)->viindoo(2) shift moves the ORM same-name edition tiebreak (ADR-0013 tier 4) so a reclassified Viindoo module ranks as Viindoo - locked by an ordering integration test.
- **New `profile_inspect(method='coverage')` (#121 P1/Rec.4):** module coverage by category for a profile, with a data-driven superset-diff (`in_profile` vs `indexed_elsewhere` = in-scope total minus in-profile) flagging categories that may be under-represented, plus a caveat routing to live-verify ("Absence from this list != absence from the product ... cross-check live `ir.module.module`"). No new tool (method on the existing discriminator, ADR-0028) - tool/resource surface unchanged (still 31 tools / 9 resources). Tenant choke ADR-0034 + read bounds ADR-0050. (Coverage tree indent corrected to ADR-0023 grammar + stale docstrings cleaned in review #348.)
- **Extended manifest metadata indexed (#121 follow-up):** nine more raw manifest keys on the Module node, same pattern as shortdesc/author (default `None`, coalesce-ON-MATCH, MERGE key untouched): `description` (full), `website`, `live_test_url`, `demo_video_url`, `support`, `sequence`, `old_technical_name`, `price`, `currency`. `installable` is intentionally NOT indexed (gate-only - every indexed node is installable). `check_module_exists` and `describe_module` now also surface `website`, `price` ("`<price> <currency>`", rendered even at 0.0), and `old_technical_name` when non-NULL; `live_test_url`/`demo_video_url`/`support`/`sequence` are index-only. `describe_module` gains an opt-in `include_description: bool = False` param: when True it appends the full manifest description as a "Description" block (default keeps the overview lean; backward compatible, tool count stays 31).
- **Ops note - re-index required:** `shortdesc`, `author`, and the recomputed `edition` are index-time properties written with coalesce-ON-MATCH (a later re-index missing the key never erases prior data). Run a `--full` re-index to backfill them (at minimum the `standard_viindoo_17` profile that contains tvtmaaddons17). The label-text change and the coverage method are read-time and live immediately after deploy - no re-index needed for those. No schema migration; the Module `(name, odoo_version)` MERGE key is untouched.
- **Zero-warning reindex + asset/report/view-condition indexing (#345, ADR-0052):** every re-index used to emit ~400 WARNING/ERROR lines; root-caused to a `.single()` same-name edge-dup bug + log-level policy + parser gaps (0 crashes). Introduces the per-feature `VersionRegistry` convention (ADR-0052: each feature group owns its own version timeline + a single dispatcher, vs one global "era"). Adds `parser_assets.py` (new `AssetBundle` node + `EXTENDS_ASSET_BUNDLE`: era-A `<template>` v8-14 / era-B manifest `assets` dict v15-19), indexes `ir.actions.report` (new `Report` node + `REPORTS_ON`/`USES_TEMPLATE`), and `attrs`/`states` + v17 direct-expression `invisible=`. The four targeted noise classes drop to 0 with no genuine model loss. New Neo4j `AssetBundle`/`Report` nodes + constraints are created by `setup_indexes()` on the first indexer run (zero migration).
- **Version-aware report-type gate (#346/#347):** the report indexing above emitted a net-new "unresolved REPORTS_ON / USES_TEMPLATE" warning class (proven 18/18 real legacy-source gaps, not binding bugs); WARN + template-binding are now gated to genuine qweb reports (v11+), while legacy RML/XSL/controller reports route to DEBUG (the node is still created). Also fixes a pre-existing bug that silently dropped legacy v8-v10 record-form `ir.actions.report.xml` reports, and routes REPORTS_ON-miss WARN/DEBUG by own-module namespace (not dotted prefix).
- **Reconcile OSM graph with on-disk Odoo source (#117):** six grounding fixes - index the curated underscore-rename method family + deprecation-marked aliases; add `res.users.has_group`/`has_groups`; deterministic core-symbol ranking + `api_version_diff` anchor for homonyms (e.g. `flush`); v18 lint rules W8193/W8205/W8213; and ACL/cache deprecated-API edges for `find_deprecated_usage`.
- **`_edition_label` all-NULL fallback (#350):** a module with neither an indexed `edition` nor `license` now reads the verbose "Community (CE) - free, bundled with Odoo CE" label, consistent with every other render path (was a bare, inconsistent "Community (CE)").
- **Dependency-closure VLP removed (#351, ADR-0048):** `module_inspect(method='dependencies')` no longer uses a variable-length `DEPENDS_ON*1..20` path (the #273-class path-enumeration risk); replaced with a per-hop name-deduped BFS bounded by a single wall-clock deadline. Output is identical to before; tenant scope stays in the Cypher choke-point (ADR-0034).
- **Build:** bump `actions/checkout` 6 -> 7 (CI workflow only).
...
- **Add `name_filter` narrowing param to `model_inspect` (#339):** new optional `name_filter: str | None` substring (case-insensitive) filter applied IN-QUERY across the `fields` and `methods` listings of `model_inspect`; `views`/`summary` silently ignore it. The filter is threaded through both the list and the count Cypher queries (`_list_fields_with_inherited`/`_count_fields_with_inherited`, `_list_methods_with_inherited`/`_count_methods_with_inherited`) so the "Showing X of N" footer reflects the post-filter total, and through the Python-side magic-field prelude so synthetic fields filter consistently. `kind` + `name_filter` combine as AND; a 0-match renders the existing `(none)` sentinel (ADR-0023 compliant, no exception). The tool docstring still points to `entity_lookup` for exact single-field lookup. No new tool - surface count unchanged. Closes #339.
- **Fix startup warning `mcp.resource_cache_ttl_seconds overlay-only resolve failed: PostgreSQL pool is not initialized`:** `register_resources(mcp)` runs at MCP module-import time (before `main()` starts the PG pool), and previously called `get_cache()` eagerly - resolving the resource-cache TTL overlay against an uninitialised pool and logging a spurious warning on every startup. `get_cache()` is now resolved lazily INSIDE each of the 9 `odoo://` resource handlers (request time, pool up by then). `get_cache()` is idempotent (double-checked lock), so per-call cost is one `is None` check; behavior is identical at request time (TTL default 300s) and the overlay value is now honoured on first use. No tool/resource surface change (still 31 tools / 9 resources).
- **Mark `mcp.resource_cache_ttl_seconds` `requires_restart=True`:** the MCP resource `_CACHE` singleton is frozen at MCP process start, and the admin-settings PATCH runs in the SEPARATE webui process - so a TTL change cannot reach the live MCP cache without an MCP restart (cross-process live-invalidation is intentionally out of scope). The admin UI now surfaces an honest "requires MCP restart" hint instead of a misleading propagation ETA.
- **Patterns-only reseed no longer runs the full-schema index setup:** `seed_patterns._write_neo4j()` called `writer.setup_indexes()` (~33 `CREATE INDEX IF NOT EXISTS` across every node label) when a patterns reseed only needs the 3 `PatternExample` indexes. Added `Neo4jWriter.setup_pattern_indexes()`, which draws its statements from a shared `_PATTERN_EXAMPLE_INDEX_STATEMENTS` module constant that `setup_indexes()` also splices in - so the two paths cannot drift (a new PatternExample index updates both; guarded by a drift test). The full indexer (`pipeline.py`, `indexer/__main__.py`) still calls `setup_indexes()` for every index on a fresh DB - unchanged.
- **Filter expected Neo4j INFORMATION `IndexOrConstraintAlreadyExists` notifications on the WRITE path:** the neo4j 5.28.4 driver logs every DBMS notification via `neo4j.notifications`; an already-indexed DB returns one INFORMATION "already exists" notice per `CREATE INDEX IF NOT EXISTS`. The indexer/write-path drivers (`writer_neo4j.py`, `pipeline.py`, `pipeline_reembed.py`) now pass `notifications_min_severity=NotificationMinimumSeverity.WARNING` - a Bolt-level server-side filter (root cause, not a Python-logger silence). The MCP READ driver (`src/mcp/server.py`) is deliberately UNFILTERED so it keeps useful INFORMATION-level performance hints (e.g. cartesian product); genuine WARNING/ERROR notifications remain logged everywhere.
- **Raise the `neo4j` floor to `>=5.14`:** `NotificationMinimumSeverity` and the `notifications_min_severity` driver kwarg (above) were added in the neo4j Python driver 5.14.0. The previous `neo4j>=5.0,<6.0` range let a non-lockfile install resolve to 5.0-5.13.x and crash at import; the floor is now `>=5.14,<6.0` (`uv.lock` re-locked; resolved version unchanged at 5.28.4). Also synced the `get_cache()`/`reset_cache()` docstrings to the new restart semantics.
- **Bootstrap converges catalogue METADATA to existing rows - owner-only (B3-root, Option B):** `register_settings_idempotent()` seeded `app_settings` with `ON CONFLICT ... DO NOTHING`, so a change to a setting's REGISTRY METADATA (e.g. flipping `requires_restart=True` for `mcp.resource_cache_ttl_seconds`, above) never reached deployments that already had the row - the metadata was frozen at first bootstrap and required a manual `UPDATE`. The function now takes a `converge_metadata` flag: `False` (default) keeps the original safe `DO NOTHING` (insert-missing-only); `True` switches to `DO UPDATE SET` that re-syncs exactly the 8 registry-derived metadata columns (`category`, `data_type`, `validation_json`, `default_value`, `requires_restart`, `requires_reseed`, `is_secret`, `description`) from the catalogue. Only the **webui** process (owner DSN) passes `converge_metadata=True`, because the admin settings LIST grid (`GET /api/admin/settings`) is the *sole* consumer of the DB-row metadata columns (for display); the **MCP** process reads only `value_json` (missing rows fall back to the in-process registry) and stays `converge_metadata=False`. The admin-set `value_json` is NEVER in the SET list, so an operator's tuned value is always preserved (ADR-0042: admin PATCH wins); `updated_at`/`updated_by`/`change_reason` and the identity columns are left untouched (a metadata convergence is a system op, not a user edit). Insert-vs-update counting is honest in both modes (`RETURNING (xmax = 0)` for converge, `rowcount` for DO NOTHING); the return value stays "rows newly inserted" and the startup log reports the mode + counts. `signup.enabled`'s env-seed-on-fresh-install behavior is unchanged (neither mode touches `value_json`). **No migration and no privilege change:** because only the owner converges, `osm_reader` keeps its existing SELECT+INSERT on `app_settings` and is deliberately granted NO UPDATE (least privilege - the public-facing read tier can never mutate settings).
- **Fixed: `--geoip-db` incorrectly marked deprecated for v8-v16:** the #338 backfill flagged `--geoip-db` as `status="deprecated"` with `replacement_flag_name="--geoip-city-db"` across ALL of v8-v18, but `--geoip-city-db` only exists from v17 onward (v17/v18 `config.py` declare `add_option("--geoip-city-db", "--geoip-db", ...)`, making `--geoip-db` an alias). In v8-v16 `--geoip-db` is the sole, live primary geoip database flag. Corrected `cli_flags_8.0.json`..`cli_flags_16.0.json` to `status="stable"` with `replacement_flag_name=null`; v17/v18 keep the deprecated-alias marking (already correct), and v19 is unchanged. Source-verified per version against `odoo{N}/.../tools/config.py`; new red-green tests in `test_spec_data_cli_flags_curated.py` pin the per-version status. No tool/resource surface change (still 31 tools / 9 resources).

## [0.17.1] - 2026-06-20 - Full v19 CLI flag index + i18n/db/module subcommands

- **Full v19 CLI flag index (#336):** indexed all Odoo v19 CLI commands and flags that were previously missing from OSM. Coverage now includes all 13 sub-actions across the 3 subparser commands (db: init/load/dump/duplicate/rename/drop; i18n: import/export/loadlang; module: install/upgrade/uninstall/force-demo), 10 non-subparser commands (scaffold/shell/start/cloc/deploy/populate/neutralize/obfuscate/upgrade_code/genproxytoken), and 7 server drift flags (--workers, --limit-memory-hard, --limit-memory-hard-gevent, --limit-memory-soft-gevent, --limit-request, --limit-time-cpu, --geoip-city-db). Total: ~99 new flag entries. Stale `tsconfig` command removed from v19 (file removed in v19 Odoo source).
- **Subparser compound-name convention:** sub-actions stored as CLICommand/CLIFlag nodes with compound command_name containing a space (e.g. `"db init"`, `"i18n export"`, `"module upgrade"`), matching the actual CLI syntax. Shared flags (e.g. `--config`, `--database`) stored under the parent command name; sub-action-specific flags under the compound name. No schema change (command_name is the existing MERGE key discriminator).
- **cli_help lists sub-commands (#336):** `cli_help(command='db')` (and i18n, module) now shows shared flags PLUS a `Sub-commands (N): ...` tree branch listing all compound sub-commands, plus a tip to call `cli_help('db init', ...)` for sub-command-specific flags. No output change for non-subparser commands or compound-name lookups.
- **v18 i18n flag deprecation markers:** 4 i18n flags removed from v19 server (`--i18n-export`, `--i18n-import`, `--language`, `--modules`) now carry `status: "deprecated"` and `replacement_flag_name` pointing to the equivalent v19 subcommand in cli_flags_18.0.json.
- **model_inspect / entity_lookup cross-link doc (#336):** `model_inspect` SKIP-when section now cross-links to `entity_lookup(kind='field', model=..., field=<exact_name>)` for single-field narrowing after `method='fields'` returns too many rows. `entity_lookup` TRIGGER section adds the symmetric pointer. No tool surface change (31/9 unchanged).

## [0.17.0] - 2026-06-19 - Persist pattern category + fix CHANGELOG drift

- **Persist pattern `category` end-to-end (#331):** the `patterns.json` column was a static hardcoded label only at seed time; now `category` is a real PostgreSQL column on patterns table (migration 0002) + a Neo4j PatternExample property set on each reseed (MERGE key remains `{pattern_id}` only). Canonical SHA -> one-time global reseed. After deploy, run `python ops/backfill_patterns.py` once to populate the `patterns.category` column (idempotent); subsequent seed cycles auto-reseed Neo4j/pgvector when the canonical SHA changes. `suggest_pattern` reads the live `category` property (no longer prefix-`test-` hack). Admin patterns API + UI filter by category; all 8 test + 121 production patterns now discoverable by category. Closes #331.
- **Fixed CHANGELOG `[0.15.0]` test-pattern id drift (Item 2 of #331):** corrected `[0.15.0]` bullet "Pattern catalogue (WI-2)" to list the 8 correct pattern ids: test-transaction-savepoint-v16plus, test-savepointcase-v8-v15, test-computed-field, test-access-rights, test-multicompany-constraint, test-httpcase-tour-qunit-v17, test-httpcase-tour-hoot-v18, test-form-onchange (was listing invalid ids from draft).
...
- **6 new MCP tools (25 -> 31):** `find_test_examples` (semantic test search, returns test chunks only), `tests_covering` (COVERS_* edge traversal), `test_class_inspect` (class/helper inspection tree), `test_base_classes` (framework base menu + cr.commit() FORBIDDEN contract), `test_coverage_audit` (unreferenced field/method audit), `js_test_inspect` (Hoot/QUnit/tour map per module). All ADR-0023 tree grammar, English-only, mandatory Next: footer, odoo_version with active-version default.
- **2 new MCP resources (7 -> 9):** `odoo://{version}/test/{module}/{class_name}` (test class definition + inheritance + methods), `odoo://{version}/testcoverage/{model}` (test coverage audit by model). LRU 1000 / 300s TTL / per-tenant keyed like existing resources.
- **Indexer pipeline (WI-1):** era2 AST + era1 regex degraded parser for Python test classes (parser_test.py); ordered + qualified base-class helper (HIGH-1 debate fix); @tagged/@standalone decorator extraction; self.env['model'] subscript def-use pass for field-level coverage precision; asserts_count counter; TestHelper framework-base seeding into odoo/tests walk per version; writer batches + reconcile passes (reconcile_test_inherits / reconcile_test_coverage / finalize is_helper); --full stale-GC for test nodes. Zero DB migration.
- **Pattern catalogue (WI-2):** 8 test-writing patterns seeded into patterns.json with test category: test-transaction-savepoint-v16plus, test-savepointcase-v8-v15, test-computed-field, test-access-rights, test-multicompany-constraint, test-httpcase-tour-qunit-v17, test-httpcase-tour-hoot-v18, test-form-onchange, all with "no cr.commit()" gotcha prominence. _SeedMeta sha256 update auto-reseeds changed patterns.
- **JavaScript test indexing (WI-3):** JsTestSuite extraction for Hoot (@odoo/hoot -> framework='hoot'), QUnit (QUnit. -> framework='qunit'), web_tour.tour (framework='tour'); per-file describe/test titles, tags, mounts, mock_models (raw, separated from edge emission). TESTS_TOUR edge targets web_tour.tours registry entries best-effort. File-level granularity only — no JS method nodes, no intra-file decomposition.
- **Tool contract hardening:** tool examples regenerated from real extracted data (WI-4 pending live v17 re-index for fresh examples; marked [illustrative shape] until then). test_base_classes always outputs cr.commit() FORBIDDEN rule (PP3 contract). test_coverage_audit lists % coverage + static-mention caveat. tests_covering groups results by via type (assert/setup/body). Concurrent subagents: pass odoo_version explicitly to avoid session pin race.
- **Incremental indexer:** `reconcile_test_inherits` / `reconcile_test_coverage` / finalize `is_helper` run VERSION-WIDE (not gated by "repo changed") next to reconcile_same_name_inherits. Stale test-node GC integrated into --full path. Multi-version incremental safe via version-keyed MERGE.
- **Site/schema sync:** `site/src/lib/constants.ts` TOOL_COUNT=31, RESOURCE_COUNT=9, SITE_VERSION=0.15.0 (test_tool_count_sync.py enforces). ADR-0051 documents C1 test-index schema, node/edge topology, merge keys (CRITICAL-1: 5-part TestClass key including repo), is_definition ranking rule for COVERS_*, era1 degraded path, framework sentinel, static-reference semantics, zero migration. ADR INDEX appended.
- **Review-fix round (PR #323 post-review):** COVERS_METHOD edges now emitted in reconcile_test_coverage (were declared but never written — tests_covering(method=...) always returned empty); gc_stale_test_nodes scoped per-repo (multi-repo profiles were cross-deleting same-named-module test nodes); incremental file-level prune restricted to re-parsed modules only (no false deletions on unchanged modules); Neo4j session leak in _list_test_classes_for_module closed; OrmQueryTimeout propagates to @offload_neo4j for metric + degraded body (was swallowed, causing false empty results); via='body' coverage rendered in tests_covering output (was computed but never emitted); full Vietnamese diacritics added to 4 tool TRIGGER blocks; _reraise_timeout=True on both new resource renderers (no-poison cache contract, ADR-0030); ADR-0051 amended to document repo in TestClass/TestMethod schema (5-part key).
- **Test: deterministic config test (PR #323 post-review):** `test_get_driver_reads_neo4j_uri_from_config` made isolation-safe via monkeypatch of `_version_checked` (was leaking state from prior test + failing outside CI on AttributeError).
