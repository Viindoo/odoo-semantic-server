...
- **A Web UI index job could stay `queued` forever (#381 F1):** a child that died before its first `running` report left a row with no pid, which the start-up sweep skipped and the reset route refused (prod: jobs 3, 4, 5). The Web UI now records the child's pid right after the spawn; `mark_dead_jobs` expires a `queued` job that never started (`started_at` NULL; any pid, since a recorded pid may be reused after a reboot or belong to another user) older than `INDEXER_JOB_QUEUED_TTL_SECONDS` (new, default 600s, env override; `docs/operations/timeouts.md`); `POST /api/jobs/{id}/reset` also accepts a `queued` job.
