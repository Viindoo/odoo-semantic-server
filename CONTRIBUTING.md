...
1–2. `DeprecationWarning: The @wait_container_is_ready decorator is deprecated...` (fires twice)  
   Source: `testcontainers/core/waiting_utils.py` (module-level `wait_for` decorated with `@wait_container_is_ready`) + `testcontainers/neo4j/__init__.py` (`_connect` method) - import/class-definition time inside the library itself, not per-test. `conftest.py` already adopts the structured `LogMessageWaitStrategy` + `_connect` override as the replacement pattern; these 2 warnings are emitted by testcontainers' own legacy machinery before any downstream code runs and cannot be suppressed without `filterwarnings` (forbidden). No testcontainers 5.0 release exists as of 2026-06 (latest stable 4.14.2); re-evaluate when testcontainers removes the `wait_container_is_ready` machinery from `waiting_utils`. Only fires under local Docker (testcontainers), not in CI (`CI=true` skips it).
