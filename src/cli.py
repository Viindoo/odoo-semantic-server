...
def _restore_sql_or_dump(path: Path, args) -> int:
    ...
    if path.suffix.lower() == ".dump":
        ...
        cmd = _resolve_postgres_tool("pg_restore") + [*pg_args, str(path)]
...
def _restore_bundle(path: Path, args) -> int:
    ...
    with tempfile.TemporaryDirectory() as _tmpdir:
        ...
        env = {**os.environ, **env_overrides}
        safety_cmd = _resolve_postgres_tool("pg_dump") + [*pg_args, "-F", "custom", "-Z", "6"]
        ...
        if pg_dump.suffix.lower() == ".dump":
            pg_restore_cmd = _resolve_postgres_tool("pg_restore") + [*pg_args, str(pg_dump)]
