...
def _resolve_stylesheet(
    ...
    with _get_driver().session() as session:
        ...
        rows = session.run(
            ...
            """,
            mod=module, v=odoo_version, **_scope(),
        ).data()
...
for _tool_mod in ("src.mcp.tools.orm_tools",):
