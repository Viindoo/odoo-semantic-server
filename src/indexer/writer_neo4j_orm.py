...
def _write_parse_result(tx, result: ParseResult, profiles: list[str]) -> None:
    ...
    tx.run(f"""
        ...
        ON CREATE SET m.profile = $profiles,
                      ...
                      m.website = $website,
                      m.live_test_url = $live_test_url,
                      m.demo_video_url = $demo_video_url,
                      m.support = $support,
                      m.sequence = $sequence,
                      m.old_technical_name = $old_technical_name,
                      m.price = $price,
                      m.currency = $currency,
        ...
        ON MATCH  SET m.profile =
                      ...
                      m.shortdesc = coalesce($shortdesc, m.shortdesc),
                      m.author = coalesce($author, m.author),
                      m.description = coalesce($description, m.description),
                      m.website = coalesce($website, m.website),
                      m.live_test_url = coalesce($live_test_url, m.live_test_url),
                      m.demo_video_url = coalesce($demo_video_url, m.demo_video_url),
                      m.support = coalesce($support, m.support),
                      m.sequence = coalesce($sequence, m.sequence),
                      m.old_technical_name =
                          coalesce($old_technical_name, m.old_technical_name),
                      m.price = coalesce($price, m.price),
                      m.currency = coalesce($currency, m.currency),
                      m.external_python = $external_python,
                      m.external_bin = $external_bin,
                      m.countries = $countries,
    ...
    """, name=module.name, v=module.odoo_version,
         ...
         live_test_url=module.live_test_url,
         demo_video_url=module.demo_video_url,
         support=module.support,
         sequence=module.sequence,
         old_technical_name=module.old_technical_name,
         price=module.price,
