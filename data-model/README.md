...
One file per table. Each file owns its schema, invariants, and example rows. Cross-table relationships live in [`../architecture/graph-store.md`](../architecture/graph-store.md).
...
Handlers query `public.<table> UNION ALL <tenant>.<table>` and resolve overrides with tenant rows winning. See [`../architecture/tenancy.md`](../architecture/tenancy.md).
